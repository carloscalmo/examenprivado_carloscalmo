...
$layout->blocks["top"][] = "details";$page_layouts["descarga_inventario_expirado_edit"] = $layout;
...
$templatefile = "descarga_inventario_expirado_edit.htm";
...
//	processing lotes - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_lotes = $pageObject->getControl("lotes", $id);
		$control_lotes->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: descarga_inventario_expirado_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: descarga_inventario_expirado_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("lotes_label","<label for=\"".GetInputElementId("lotes", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='descarga_inventario_expirado_view.php?".$viewlink."'\"");
