...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["empleados_cirugia_detailspreview"] = $layout;
...
if($mastertable == "cirugia")
{
	$where = "";
		$where .= GetFullFieldName("cirugia_id", $strTableName, false)."=".make_db_value("cirugia_id",$_SESSION[$strTableName."_masterkey1"]);
}
if($mastertable == "empleado")
{
	$where = "";
		$where .= GetFullFieldName("empleado_id", $strTableName, false)."=".make_db_value("empleado_id",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	empleado_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("empleado_id", $data, $keylink);
			$row["empleado_id_value"] = $value;
	//	cirugia_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("cirugia_id", $data, $keylink);
			$row["cirugia_id_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("empleados_cirugia_detailspreview.htm");
