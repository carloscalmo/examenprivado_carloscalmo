...
add_nocache_headers();
include("include/cirugia_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["cirugia_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "cirugia_inline_add.htm";
else
	$templatefile = "cirugia_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: cirugia_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing idtipo_cirugia - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_idtipo_cirugia = $pageObject->getControl("idtipo_cirugia", $id);
		$control_idtipo_cirugia->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing idtipo_cirugia - end
...
//	processing encargado - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_encargado = $pageObject->getControl("encargado", $id);
		$control_encargado->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing encargado - end
//	processing ambiente_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_ambiente_id = $pageObject->getControl("ambiente_id", $id);
		$control_ambiente_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ambiente_id - end
...
//	insert masterkey value if exists and if not specified
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
	{
		...
		if($avalues["encargado"]==""){
			$avalues["encargado"] = prepare_for_db("encargado",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="tipo_cirugia")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["idtipo_cirugia"]==""){
			$avalues["idtipo_cirugia"] = prepare_for_db("idtipo_cirugia",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="ambiente")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["ambiente_id"]==""){
			$avalues["ambiente_id"] = prepare_for_db("ambiente_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing idtipo_cirugia - start
			...
			if($inlineAddOption)
			{
				$control_idtipo_cirugia->afterSuccessfulSave();
			}
//	processing idtipo_cirugia - end
//	processing consulta_id - start
			$inlineAddOption = true;
...
//	processing encargado - start
			...
			if($inlineAddOption)
			{
				$control_encargado->afterSuccessfulSave();
			}
//	processing encargado - end
//	processing ambiente_id - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_ambiente_id->afterSuccessfulSave();
			}
//	processing ambiente_id - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'cirugia_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'cirugia_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: cirugia_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["consulta_id"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["consulta_id"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["encargado"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="tipo_cirugia")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["idtipo_cirugia"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="ambiente")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["ambiente_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["encargado"]=@$avalues["encargado"];
	$defvalues["ambiente_id"]=@$avalues["ambiente_id"];
	$defvalues["idtipo_cirugia"]=@$avalues["idtipo_cirugia"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("encargado"))
		$xt->assign("encargado_fieldblock",true);
	else
		$xt->assign("encargado_tabfieldblock",true);
	$xt->assign("encargado_label",true);
	if(isEnableSection508())
		$xt->assign_section("encargado_label","<label for=\"".GetInputElementId("encargado", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("ambiente_id"))
		$xt->assign("ambiente_id_fieldblock",true);
	else
		$xt->assign("ambiente_id_tabfieldblock",true);
	$xt->assign("ambiente_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("ambiente_id_label","<label for=\"".GetInputElementId("ambiente_id", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("idtipo_cirugia"))
		$xt->assign("idtipo_cirugia_fieldblock",true);
	else
		$xt->assign("idtipo_cirugia_tabfieldblock",true);
	$xt->assign("idtipo_cirugia_label",true);
	if(isEnableSection508())
		$xt->assign_section("idtipo_cirugia_label","<label for=\"".GetInputElementId("idtipo_cirugia", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["empleados_cirugia"]["masterkey1"] = $data["id"];	
...
//	encargado
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("encargado", $data, $keylink);
		$showValues["encargado"] = $value;
		$showFields[] = "encargado";
	}	
//	ambiente_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("ambiente_id", $data, $keylink);
		$showValues["ambiente_id"] = $value;
		$showFields[] = "ambiente_id";
	}	
//	idtipo_cirugia
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("idtipo_cirugia", $data, $keylink);
		$showValues["idtipo_cirugia"] = $value;
		$showFields[] = "idtipo_cirugia";
	}	
//	consulta_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showRawValues["encargado"] = substr($data["encargado"],0,100);
		$showRawValues["ambiente_id"] = substr($data["ambiente_id"],0,100);
		$showRawValues["idtipo_cirugia"] = substr($data["idtipo_cirugia"],0,100);
