...
add_nocache_headers();
include("include/lotes_variables.php");
...
{
	echo "<p>"."No tiene permiso para acceder a esta tabla"."<br>Proceed to <a href=\"admin.php\">Admin Area</a> to set up user permissions</p>";
	return;
}
...
$layout = new TLayout("add3","Rounded1BlueWave","MobileBlueWave");
...
$layout->blocks["top"][] = "details";$page_layouts["lotes_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "lotes_inline_add.htm";
else
	$templatefile = "lotes_add.htm";
...
//	processing codigo - start
	...
	if($inlineAddOption)
	{
		...
		$control_codigo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing codigo - end
//	processing produccion - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_produccion = $pageObject->getControl("produccion", $id);
		$control_produccion->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing produccion - end
//	processing caducidad - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_caducidad = $pageObject->getControl("caducidad", $id);
		$control_caducidad->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing caducidad - end
//	processing expirado - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_expirado = $pageObject->getControl("expirado", $id);
		$control_expirado->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing expirado - end
//	processing proveedor - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_proveedor = $pageObject->getControl("proveedor", $id);
		$control_proveedor->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing proveedor - end
//	processing insumo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_insumo = $pageObject->getControl("insumo", $id);
		$control_insumo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing insumo - end
//	processing cantidad - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_cantidad = $pageObject->getControl("cantidad", $id);
		$control_cantidad->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing cantidad - end
//	processing precio - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_precio = $pageObject->getControl("precio", $id);
		$control_precio->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing precio - end
//	processing costo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_costo = $pageObject->getControl("costo", $id);
		$control_costo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing costo - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="insumos")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["insumo"]==""){
			$avalues["insumo"] = prepare_for_db("insumo",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="proveedores")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["proveedor"]==""){
			$avalues["proveedor"] = prepare_for_db("proveedor",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing produccion - start
			...
			if($inlineAddOption)
			{
				$control_produccion->afterSuccessfulSave();
			}
//	processing produccion - end
//	processing caducidad - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_caducidad->afterSuccessfulSave();
			}
//	processing caducidad - end
//	processing expirado - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_expirado->afterSuccessfulSave();
			}
//	processing expirado - end
//	processing proveedor - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_proveedor->afterSuccessfulSave();
			}
//	processing proveedor - end
//	processing insumo - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_insumo->afterSuccessfulSave();
			}
//	processing insumo - end
//	processing cantidad - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_cantidad->afterSuccessfulSave();
			}
//	processing cantidad - end
//	processing precio - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_precio->afterSuccessfulSave();
			}
//	processing precio - end
//	processing costo - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_costo->afterSuccessfulSave();
			}
//	processing costo - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'lotes_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'lotes_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: lotes_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="insumos")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["insumo"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="proveedores")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["proveedor"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["produccion"]=@$avalues["produccion"];
	$defvalues["caducidad"]=@$avalues["caducidad"];
	$defvalues["expirado"]=@$avalues["expirado"];
	$defvalues["proveedor"]=@$avalues["proveedor"];
	$defvalues["insumo"]=@$avalues["insumo"];
	$defvalues["cantidad"]=@$avalues["cantidad"];
	$defvalues["precio"]=@$avalues["precio"];
	$defvalues["costo"]=@$avalues["costo"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("codigo_label","<label for=\"".GetInputElementId("codigo", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("produccion"))
		$xt->assign("produccion_fieldblock",true);
	else
		$xt->assign("produccion_tabfieldblock",true);
	$xt->assign("produccion_label",true);
	if(isEnableSection508())
		$xt->assign_section("produccion_label","<label for=\"".GetInputElementId("produccion", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("caducidad"))
		$xt->assign("caducidad_fieldblock",true);
	else
		$xt->assign("caducidad_tabfieldblock",true);
	$xt->assign("caducidad_label",true);
	if(isEnableSection508())
		$xt->assign_section("caducidad_label","<label for=\"".GetInputElementId("caducidad", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("expirado"))
		$xt->assign("expirado_fieldblock",true);
	else
		$xt->assign("expirado_tabfieldblock",true);
	$xt->assign("expirado_label",true);
	if(isEnableSection508())
		$xt->assign_section("expirado_label","<label for=\"".GetInputElementId("expirado", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("proveedor"))
		$xt->assign("proveedor_fieldblock",true);
	else
		$xt->assign("proveedor_tabfieldblock",true);
	$xt->assign("proveedor_label",true);
	if(isEnableSection508())
		$xt->assign_section("proveedor_label","<label for=\"".GetInputElementId("proveedor", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("insumo"))
		$xt->assign("insumo_fieldblock",true);
	else
		$xt->assign("insumo_tabfieldblock",true);
	$xt->assign("insumo_label",true);
	if(isEnableSection508())
		$xt->assign_section("insumo_label","<label for=\"".GetInputElementId("insumo", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("cantidad_label","<label for=\"".GetInputElementId("cantidad", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("precio"))
		$xt->assign("precio_fieldblock",true);
	else
		$xt->assign("precio_tabfieldblock",true);
	$xt->assign("precio_label",true);
	if(isEnableSection508())
		$xt->assign_section("precio_label","<label for=\"".GetInputElementId("precio", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("costo"))
		$xt->assign("costo_fieldblock",true);
	else
		$xt->assign("costo_tabfieldblock",true);
	$xt->assign("costo_label",true);
	if(isEnableSection508())
		$xt->assign_section("costo_label","<label for=\"".GetInputElementId("costo", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["detalle_compra"]["masterkey1"] = $data["id"];	
	$showDetailKeys["solicitud_medicamento"]["masterkey1"] = $data["id"];	
...
//	produccion
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("produccion", $data, $keylink);
		$showValues["produccion"] = $value;
		$showFields[] = "produccion";
	}	
//	caducidad
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("caducidad", $data, $keylink);
		$showValues["caducidad"] = $value;
		$showFields[] = "caducidad";
	}	
//	expirado
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("expirado", $data, $keylink);
		$showValues["expirado"] = $value;
		$showFields[] = "expirado";
	}	
//	proveedor
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("proveedor", $data, $keylink);
		$showValues["proveedor"] = $value;
		$showFields[] = "proveedor";
	}	
//	insumo
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("insumo", $data, $keylink);
		$showValues["insumo"] = $value;
		$showFields[] = "insumo";
	}	
//	cantidad
	$display = false;
...
//	precio
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("precio", $data, $keylink);
		$showValues["precio"] = $value;
		$showFields[] = "precio";
	}	
//	costo
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("costo", $data, $keylink);
		$showValues["costo"] = $value;
		$showFields[] = "costo";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["codigo"] = substr($data["codigo"],0,100);
		$showRawValues["produccion"] = substr($data["produccion"],0,100);
		$showRawValues["caducidad"] = substr($data["caducidad"],0,100);
		$showRawValues["expirado"] = substr($data["expirado"],0,100);
		$showRawValues["proveedor"] = substr($data["proveedor"],0,100);
		$showRawValues["insumo"] = substr($data["insumo"],0,100);
		$showRawValues["cantidad"] = substr($data["cantidad"],0,100);
		$showRawValues["precio"] = substr($data["precio"],0,100);
		$showRawValues["costo"] = substr($data["costo"],0,100);
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="detalle_compra" && postvalue("field")=="lote") 
	{
		$lookupFieldName = "lote";
		$lookupTable = "lotes";
		$linkFieldName = "id";
		$dispfield = "codigo";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("detalle_compra", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "detalle_compra", "lote");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("id", $strTableName, true);
