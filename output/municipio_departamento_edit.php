...
$layout->blocks["top"][] = "details";$page_layouts["municipio_departamento_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "municipio_departamento_inline_edit.htm" : "municipio_departamento_edit.htm";
...
//	processing departamento - begin
	$condition = 1;

	if($condition)
	{
		$control_departamento = $pageObject->getControl("departamento", $id);
		$control_departamento->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

	}
//	processing departamento - end
//	processing Municipio - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_Municipio = $pageObject->getControl("Municipio", $id);
		$control_Municipio->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: municipio_departamento_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: municipio_departamento_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["departamento"] = $evalues["departamento"];
	$data["Municipio"] = $evalues["Municipio"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("departamento_label","<label for=\"".GetInputElementId("departamento", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='municipio_departamento_view.php?".$viewlink."'\"");
