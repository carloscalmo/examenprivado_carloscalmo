...
$layout->blocks["top"][] = "details";$page_layouts["empleado_edit"] = $layout;
...
$templatefile = "empleado_edit.htm";
...
$keys["persona_id"] = urldecode(postvalue("editid2"));
$savedKeys["persona_id"] = urldecode(postvalue("editid2"));
...
//	processing profesion - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_profesion = $pageObject->getControl("profesion", $id);
		$control_profesion->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing profesion - end
//	processing puesto_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_puesto_id = $pageObject->getControl("puesto_id", $id);
		$control_puesto_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing puesto_id - end
//	processing persona_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_persona_id = $pageObject->getControl("persona_id", $id);
		$control_persona_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_persona_id->getWebValue()!==false)
			$keys["persona_id"] = $control_persona_id->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["persona_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: empleado_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: empleado_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["profesion"] = $evalues["profesion"];
	$data["puesto_id"] = $evalues["puesto_id"];
	$data["persona_id"] = $evalues["persona_id"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("profesion_label","<label for=\"".GetInputElementId("profesion", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("puesto_id_label","<label for=\"".GetInputElementId("puesto_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("persona_id_label","<label for=\"".GetInputElementId("persona_id", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("id", $data)));
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("persona_id", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='empleado_view.php?".$viewlink."'\"");
