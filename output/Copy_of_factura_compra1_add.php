...
add_nocache_headers();
include("include/Copy_of_factura_compra1_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["Copy_of_factura_compra1_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Copy_of_factura_compra1_inline_add.htm";
else
	$templatefile = "Copy_of_factura_compra1_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Copy_of_factura_compra1_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Copy_of_factura_compra1_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Copy_of_factura_compra1_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Copy_of_factura_compra1_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Copy of factura_compra1";
		$options['firstTime'] = 1;
