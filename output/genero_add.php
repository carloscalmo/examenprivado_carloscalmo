...
add_nocache_headers();
include("include/genero_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["genero_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "genero_inline_add.htm";
else
	$templatefile = "genero_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: genero_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'genero_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'genero_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: genero_".$pageObject->getPageType().".php");
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="persona" && postvalue("field")=="genero_id") 
	{
		$lookupFieldName = "genero_id";
		$lookupTable = "genero";
		$linkFieldName = "id";
		$dispfield = "nombre";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "persona", "genero_id");
