...
$layout->blocks["top"][] = "export";$page_layouts["Copy_of_departamento_export"] = $layout;
...
$xt->display("Copy_of_departamento_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Copy_of_departamento.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_departamento.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_departamento.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["departamento"] = $pageObject->showDBValue("departamento", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_departamento.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["departamento"] = $pageObject->getViewControl("departamento")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["departamento"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nombre1").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Departamento").'</td>';	
	}
	else
	{
		echo "<td>"."Código"."</td>";
