...
add_nocache_headers();
include("include/detalle_cirugia_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["detalle_cirugia_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "detalle_cirugia_inline_add.htm";
else
	$templatefile = "detalle_cirugia_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: detalle_cirugia_add.php');
...
//	processing insumos_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_insumos_id = $pageObject->getControl("insumos_id", $id);
		$control_insumos_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing insumos_id - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="insumos")
	{
		...
		if($avalues["insumos_id"]==""){
			$avalues["insumos_id"] = prepare_for_db("insumos_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing insumos_id - start
			...
			if($inlineAddOption)
			{
				$control_insumos_id->afterSuccessfulSave();
			}
//	processing insumos_id - end
//	processing cantidad - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'detalle_cirugia_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'detalle_cirugia_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: detalle_cirugia_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["idtipo_cirugia"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["idtipo_cirugia"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="insumos")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["insumos_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["insumos_id"]=@$avalues["insumos_id"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("insumos_id"))
		$xt->assign("insumos_id_fieldblock",true);
	else
		$xt->assign("insumos_id_tabfieldblock",true);
	$xt->assign("insumos_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("insumos_id_label","<label for=\"".GetInputElementId("insumos_id", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink.="&key2=".htmlspecialchars(rawurlencode(@$data["idtipo_cirugia"]));
...
//	insumos_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("insumos_id", $data, $keylink);
		$showValues["insumos_id"] = $value;
		$showFields[] = "insumos_id";
	}	
//	cantidad
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showRawValues["insumos_id"] = substr($data["insumos_id"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "detalle_cirugia";
		$options['firstTime'] = 1;
