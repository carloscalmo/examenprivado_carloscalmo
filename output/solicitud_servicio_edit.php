...
$layout->blocks["top"][] = "details";$page_layouts["solicitud_servicio_edit"] = $layout;
...
$templatefile = "solicitud_servicio_edit.htm";
...
//	processing servicio_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_servicio_id = $pageObject->getControl("servicio_id", $id);
		$control_servicio_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing servicio_id - end
//	processing solicitante - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_solicitante = $pageObject->getControl("solicitante", $id);
		$control_solicitante->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: solicitud_servicio_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: solicitud_servicio_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["servicio_id"] = $evalues["servicio_id"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("servicio_id_label","<label for=\"".GetInputElementId("servicio_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("solicitante_label","<label for=\"".GetInputElementId("solicitante", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='solicitud_servicio_view.php?".$viewlink."'\"");
