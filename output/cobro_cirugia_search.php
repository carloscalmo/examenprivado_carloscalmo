...
include("include/cobro_cirugia_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["cobro_cirugia_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "cobro_cirugia_search.htm";
...
$params['shortTableName'] = 'cobro_cirugia';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("'Tipo de Intervención Quirurgica'"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("'Tipo de Intervención Quirurgica'")] = GetTableURL($pageObject->pSet->getLookupTable("'Tipo de Intervención Quirurgica'"));
	
	$pageObject->fillFieldToolTips("'Tipo de Intervención Quirurgica'");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("'Tipo de Intervención Quirurgica'");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "'Tipo de Intervención Quirurgica'";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("_Tipo_de_Intervenci_n_Quirurgica__label","<label for=\"".GetInputElementId("'Tipo de Intervención Quirurgica'", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("_Tipo_de_Intervenci_n_Quirurgica__label", true);
	
	$xt->assign("_Tipo_de_Intervenci_n_Quirurgica__fieldblock", true);
	$xt->assignbyref("_Tipo_de_Intervenci_n_Quirurgica__editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("_Tipo_de_Intervenci_n_Quirurgica__notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("_Tipo_de_Intervenci_n_Quirurgica__editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype__Tipo_de_Intervenci_n_Quirurgica_", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("'Tipo de Intervención Quirurgica'");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"'Tipo de Intervención Quirurgica'", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"'Tipo de Intervención Quirurgica'", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if(isEnableSection508())
		$xt->assign_section("_Precio__label","<label for=\"".GetInputElementId("'Precio'", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("_Precio__label", true);
	
	$xt->assign("_Precio__fieldblock", true);
	$xt->assignbyref("_Precio__editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("_Precio__notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("_Precio__editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype__Precio_", $ctrlBlockArr['searchtype']);
