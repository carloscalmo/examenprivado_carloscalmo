...
$layout->blocks["top"][] = "details";$page_layouts["empleados_cirugia_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "empleados_cirugia_inline_add.htm";
else
	$templatefile = "empleados_cirugia_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: empleados_cirugia_add.php');
...
//	processing cirugia_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_cirugia_id = $pageObject->getControl("cirugia_id", $id);
		$control_cirugia_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing cirugia_id - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="cirugia")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["cirugia_id"]==""){
			$avalues["cirugia_id"] = prepare_for_db("cirugia_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing cirugia_id - start
			...
			if($inlineAddOption)
			{
				$control_cirugia_id->afterSuccessfulSave();
			}
//	processing cirugia_id - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'empleados_cirugia_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'empleados_cirugia_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: empleados_cirugia_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["cirugia_id"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["cirugia_id"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="cirugia")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["cirugia_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["cirugia_id"]=@$avalues["cirugia_id"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("cirugia_id"))
		$xt->assign("cirugia_id_fieldblock",true);
	else
		$xt->assign("cirugia_id_tabfieldblock",true);
	$xt->assign("cirugia_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("cirugia_id_label","<label for=\"".GetInputElementId("cirugia_id", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink.="&key2=".htmlspecialchars(rawurlencode(@$data["cirugia_id"]));
...
//	cirugia_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("cirugia_id", $data, $keylink);
		$showValues["cirugia_id"] = $value;
		$showFields[] = "cirugia_id";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["empleado_id"] = substr($data["empleado_id"],0,100);
		$showRawValues["cirugia_id"] = substr($data["cirugia_id"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "empleados_cirugia";
		$options['firstTime'] = 1;
