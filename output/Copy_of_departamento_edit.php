...
$layout->blocks["top"][] = "details";$page_layouts["Copy_of_departamento_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Copy_of_departamento_inline_edit.htm" : "Copy_of_departamento_edit.htm";
...
//	processing id1 - end
//	processing nombre1 - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_nombre1 = $pageObject->getControl("nombre1", $id);
		$control_nombre1->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Copy_of_departamento_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Copy_of_departamento_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Copy_of_departamento_view.php?".$viewlink."'\"");
