...
$layout->blocks["top"][] = "pdf";$page_layouts["proveedores_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		foreach(@$_REQUEST["mdelete"] as $ind)
		{
			...
			$keys["nit"]=refine($_REQUEST["mdelete3"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
//	descripcion - 
			$record["descripcion_value"] = $pageObject->showDBValue("descripcion", $data, $keylink);
			$record["descripcion_class"] = $pageObject->fieldClass("descripcion");
//	saldo - Number
			$record["saldo_value"] = $pageObject->showDBValue("saldo", $data, $keylink);
			$record["saldo_class"] = $pageObject->fieldClass("saldo");
			if($col<$colsonpage)
...
//	include proper masterprint.php code
	...
	$params = array("detailtable"=>"proveedores","keys"=>$masterkeys);
	$master = array();
...
$xt->assign("saldo_fieldheadercolumn",true);
$xt->assign("saldo_fieldheader",true);
$xt->assign("saldo_fieldcolumn",true);
$xt->assign("saldo_fieldfootercolumn",true);
