...
$layout->blocks["top"][] = "pdf";$page_layouts["lotes_print"] = $layout;
...
$arr = array();
$arr['fName'] = "insumo";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("insumo");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "cantidad";
...
$arr = array();
$arr['fName'] = "costo";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("costo");
$fieldsArr[] = $arr;
...
//	insumo - 
			$record["insumo_value"] = $pageObject->showDBValue("insumo", $data, $keylink);
			$record["insumo_class"] = $pageObject->fieldClass("insumo");
//	cantidad - Number
...
//	costo - Number
			$record["costo_value"] = $pageObject->showDBValue("costo", $data, $keylink);
			$record["costo_class"] = $pageObject->fieldClass("costo");
			if($col<$colsonpage)
...
//	include proper masterprint.php code
	include("include/insumos_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"lotes","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_insumos";
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["insumos_masterprint"] = $layout;
	...
	$layout = GetPageLayout("insumos", 'masterprint');
	if($layout)
	{
...
if($mastertable == "proveedores")
{
//	include proper masterprint.php code
	include("include/proveedores_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"lotes","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_proveedores";
	$master["params"] = $params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$layout = new TLayout("masterprint","Rounded1BlueWave","MobileBlueWave");
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["proveedores_masterprint"] = $layout;
	...
	$layout = GetPageLayout("proveedores", 'masterprint');
	if($layout)
	{
		$rtl = $pageObject->xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
		$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
$xt->assign("insumo_fieldheadercolumn",true);
$xt->assign("insumo_fieldheader",true);
$xt->assign("insumo_fieldcolumn",true);
$xt->assign("insumo_fieldfootercolumn",true);
...
$xt->assign("costo_fieldheadercolumn",true);
$xt->assign("costo_fieldheader",true);
$xt->assign("costo_fieldcolumn",true);
$xt->assign("costo_fieldfootercolumn",true);
