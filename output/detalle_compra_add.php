...
add_nocache_headers();
include("include/detalle_compra_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["detalle_compra_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "detalle_compra_inline_add.htm";
else
	$templatefile = "detalle_compra_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: detalle_compra_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing producto - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_producto = $pageObject->getControl("producto", $id);
		$control_producto->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing producto - end
//	processing lote - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_lote = $pageObject->getControl("lote", $id);
		$control_lote->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing lote - end
...
//	processing factura_compra - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_factura_compra = $pageObject->getControl("factura_compra", $id);
		$control_factura_compra->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing factura_compra - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="insumos")
	{
		...
		if($avalues["producto"]==""){
			$avalues["producto"] = prepare_for_db("producto",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="factura_compra")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["factura_compra"]==""){
			$avalues["factura_compra"] = prepare_for_db("factura_compra",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="lotes")
	{
		...
		if($avalues["lote"]==""){
			$avalues["lote"] = prepare_for_db("lote",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing producto - start
			...
			if($inlineAddOption)
			{
				$control_producto->afterSuccessfulSave();
			}
//	processing producto - end
//	processing lote - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_lote->afterSuccessfulSave();
			}
//	processing lote - end
//	processing cantidad - start
			$inlineAddOption = true;
...
//	processing factura_compra - start
			...
			if($inlineAddOption)
			{
				$control_factura_compra->afterSuccessfulSave();
			}
//	processing factura_compra - end
//	processing costo - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'detalle_compra_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'detalle_compra_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: detalle_compra_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["factura_compra"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["factura_compra"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="insumos")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["producto"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="factura_compra")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["factura_compra"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="lotes")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["lote"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["producto"]=@$avalues["producto"];
	$defvalues["lote"]=@$avalues["lote"];
	$defvalues["cantidad"]=@$avalues["cantidad"];
	$defvalues["factura_compra"]=@$avalues["factura_compra"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("producto"))
		$xt->assign("producto_fieldblock",true);
	else
		$xt->assign("producto_tabfieldblock",true);
	$xt->assign("producto_label",true);
	if(isEnableSection508())
		$xt->assign_section("producto_label","<label for=\"".GetInputElementId("producto", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("lote"))
		$xt->assign("lote_fieldblock",true);
	else
		$xt->assign("lote_tabfieldblock",true);
	$xt->assign("lote_label",true);
	if(isEnableSection508())
		$xt->assign_section("lote_label","<label for=\"".GetInputElementId("lote", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("factura_compra"))
		$xt->assign("factura_compra_fieldblock",true);
	else
		$xt->assign("factura_compra_tabfieldblock",true);
	$xt->assign("factura_compra_label",true);
	if(isEnableSection508())
		$xt->assign_section("factura_compra_label","<label for=\"".GetInputElementId("factura_compra", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink.="&key2=".htmlspecialchars(rawurlencode(@$data["factura_compra"]));
...
//	producto
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("producto", $data, $keylink);
		$showValues["producto"] = $value;
		$showFields[] = "producto";
	}	
//	lote
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("lote", $data, $keylink);
		$showValues["lote"] = $value;
		$showFields[] = "lote";
	}	
//	cantidad
	$display = false;
...
//	factura_compra
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("factura_compra", $data, $keylink);
		$showValues["factura_compra"] = $value;
		$showFields[] = "factura_compra";
	}	
//	costo
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showRawValues["producto"] = substr($data["producto"],0,100);
		$showRawValues["lote"] = substr($data["lote"],0,100);
		$showRawValues["cantidad"] = substr($data["cantidad"],0,100);
		$showRawValues["factura_compra"] = substr($data["factura_compra"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "detalle_compra";
		$options['firstTime'] = 1;
