...
$layout->blocks["top"][] = "search";$page_layouts["lotes_search"] = $layout;
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("insumo"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("insumo")] = GetTableURL($pageObject->pSet->getLookupTable("insumo"));
	
	$pageObject->fillFieldToolTips("insumo");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("insumo");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("insumo_label","<label for=\"".GetInputElementId("insumo", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("insumo_label", true);
	
	$xt->assign("insumo_fieldblock", true);
	$xt->assignbyref("insumo_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("insumo_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("insumo_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_insumo", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("insumo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"insumo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"insumo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
