...
$layout->blocks["top"][] = "details";$page_layouts["proveedores_edit"] = $layout;
...
$templatefile = "proveedores_edit.htm";
...
$keys["nit"] = urldecode(postvalue("editid3"));
$savedKeys["nit"] = urldecode(postvalue("editid3"));
...
//	processing saldo - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_saldo = $pageObject->getControl("saldo", $id);
		$control_saldo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing nit - begin
	...
	if($condition)
	{
		...
		$control_nit->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
				...
				//	update key value
		if($control_nit->getWebValue()!==false)
			$keys["nit"] = $control_nit->getWebValue();
	}
//	processing nit - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid3=".rawurldecode($keys["nit"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: proveedores_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("saldo_label","<label for=\"".GetInputElementId("saldo", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='proveedores_view.php?".$viewlink."'\"");
