...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["detalle_cirugia_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("insumos_id", $strTableName, false)."=".make_db_value("insumos_id",$_SESSION[$strTableName."_masterkey1"]);
}
if($mastertable == "tipo_cirugia")
{
	$where = "";
		$where .= GetFullFieldName("idtipo_cirugia", $strTableName, false)."=".make_db_value("idtipo_cirugia",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	idtipo_cirugia - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idtipo_cirugia", $data, $keylink);
			$row["idtipo_cirugia_value"] = $value;
	//	insumos_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("insumos_id", $data, $keylink);
			$row["insumos_id_value"] = $value;
	//	cantidad - Number
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template("detalle_cirugia_detailspreview.htm");
