...
add_nocache_headers();
include("include/abonos_proveedores_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["abonos_proveedores_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "abonos_proveedores_inline_add.htm";
else
	$templatefile = "abonos_proveedores_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: abonos_proveedores_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing factura - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_factura = $pageObject->getControl("factura", $id);
		$control_factura->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing factura - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="factura_compra")
	{
		...
		if($avalues["factura"]==""){
			$avalues["factura"] = prepare_for_db("factura",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing factura - start
			...
			if($inlineAddOption)
			{
				$control_factura->afterSuccessfulSave();
			}
//	processing factura - end
//	processing fecha - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'abonos_proveedores_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'abonos_proveedores_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: abonos_proveedores_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["factura"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["factura"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="factura_compra")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["factura"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["factura"]=@$avalues["factura"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("factura"))
		$xt->assign("factura_fieldblock",true);
	else
		$xt->assign("factura_tabfieldblock",true);
	$xt->assign("factura_label",true);
	if(isEnableSection508())
		$xt->assign_section("factura_label","<label for=\"".GetInputElementId("factura", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink.="&key2=".htmlspecialchars(rawurlencode(@$data["factura"]));
...
//	factura
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("factura", $data, $keylink);
		$showValues["factura"] = $value;
		$showFields[] = "factura";
	}	
		...
		$showRawValues["factura"] = substr($data["factura"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "abonos_proveedores";
		$options['firstTime'] = 1;
