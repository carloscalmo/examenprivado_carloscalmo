...
$layout->blocks["top"][] = "export";$page_layouts["Historial_de_compras_export"] = $layout;
...
$xt->display("Historial_de_compras_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Historial_de_compras.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Historial_de_compras.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Historial_de_compras.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["numero_factura"] = $pageObject->showDBValue("numero_factura", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Historial_de_compras.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["numero_factura"] = $pageObject->getViewControl("numero_factura")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["numero_factura"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Código de Barras").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nombre").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Fecha").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Numero Factura").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["numero_factura"]);
					else
						echo $values["numero_factura"];
			echo '</td>';
							echo '<td>';
