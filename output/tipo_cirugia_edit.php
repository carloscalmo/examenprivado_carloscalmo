...
$layout->blocks["top"][] = "details";$page_layouts["tipo_cirugia_edit"] = $layout;
...
$templatefile = "tipo_cirugia_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["idtipo_cirugia"] = urldecode(postvalue("editid1"));
$savedKeys["idtipo_cirugia"] = urldecode(postvalue("editid1"));
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idtipo_cirugia"])."&";
		$keyGetQ.="editid2=".rawurldecode($keys["nombre"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: tipo_cirugia_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: tipo_cirugia_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("idtipo_cirugia", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tipo_cirugia_view.php?".$viewlink."'\"");
