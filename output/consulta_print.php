...
$layout->blocks["top"][] = "pdf";$page_layouts["consulta_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		foreach(@$_REQUEST["mdelete"] as $ind)
		{
			...
			$keys["paciente"]=refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
//	tipo_consulta_id - 
			$record["tipo_consulta_id_value"] = $pageObject->showDBValue("tipo_consulta_id", $data, $keylink);
			$record["tipo_consulta_id_class"] = $pageObject->fieldClass("tipo_consulta_id");
//	paciente - 
			$record["paciente_value"] = $pageObject->showDBValue("paciente", $data, $keylink);
			$record["paciente_class"] = $pageObject->fieldClass("paciente");
//	fecha - Short Date
			$record["fecha_value"] = $pageObject->showDBValue("fecha", $data, $keylink);
			$record["fecha_class"] = $pageObject->fieldClass("fecha");
//	medico_encargado - 
			$record["medico_encargado_value"] = $pageObject->showDBValue("medico_encargado", $data, $keylink);
			$record["medico_encargado_class"] = $pageObject->fieldClass("medico_encargado");
//	diagnostico_id - 
			$record["diagnostico_id_value"] = $pageObject->showDBValue("diagnostico_id", $data, $keylink);
			$record["diagnostico_id_class"] = $pageObject->fieldClass("diagnostico_id");
//	estado_retirada - 
			$record["estado_retirada_value"] = $pageObject->showDBValue("estado_retirada", $data, $keylink);
			$record["estado_retirada_class"] = $pageObject->fieldClass("estado_retirada");
			if($col<$colsonpage)
...
if($mastertable == "diagnostico")
{
//	include proper masterprint.php code
	include("include/diagnostico_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"consulta","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_diagnostico";
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["diagnostico_masterprint"] = $layout;

	$layout = GetPageLayout("diagnostico", 'masterprint');
	if($layout)
	{
...
if($mastertable == "persona")
{
//	include proper masterprint.php code
	include("include/persona_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"consulta","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_persona";
	$master["params"] = $params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$layout = new TLayout("masterprint","Rounded1BlueWave","MobileBlueWave");
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["persona_masterprint"] = $layout;
	...
	$layout = GetPageLayout("persona", 'masterprint');
	if($layout)
	{
		$rtl = $pageObject->xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
		$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
if($mastertable == "tipo_consulta")
{
//	include proper masterprint.php code
	include("include/tipo_consulta_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"consulta","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_tipo_consulta";
	$master["params"] = $params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$layout = new TLayout("masterprint","Rounded1BlueWave","MobileBlueWave");
...
$layout->skins["mastergrid"] = "grid";
$layout->blocks["bare"][] = "mastergrid";$page_layouts["tipo_consulta_masterprint"] = $layout;

	$layout = GetPageLayout("tipo_consulta", 'masterprint');
	if($layout)
	{
		$rtl = $pageObject->xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
		$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
$xt->assign("tipo_consulta_id_fieldheadercolumn",true);
$xt->assign("tipo_consulta_id_fieldheader",true);
$xt->assign("tipo_consulta_id_fieldcolumn",true);
$xt->assign("tipo_consulta_id_fieldfootercolumn",true);
$xt->assign("paciente_fieldheadercolumn",true);
$xt->assign("paciente_fieldheader",true);
$xt->assign("paciente_fieldcolumn",true);
$xt->assign("paciente_fieldfootercolumn",true);
...
$xt->assign("medico_encargado_fieldheadercolumn",true);
$xt->assign("medico_encargado_fieldheader",true);
$xt->assign("medico_encargado_fieldcolumn",true);
$xt->assign("medico_encargado_fieldfootercolumn",true);
$xt->assign("diagnostico_id_fieldheadercolumn",true);
$xt->assign("diagnostico_id_fieldheader",true);
$xt->assign("diagnostico_id_fieldcolumn",true);
$xt->assign("diagnostico_id_fieldfootercolumn",true);
$xt->assign("estado_retirada_fieldheadercolumn",true);
$xt->assign("estado_retirada_fieldheader",true);
$xt->assign("estado_retirada_fieldcolumn",true);
$xt->assign("estado_retirada_fieldfootercolumn",true);
