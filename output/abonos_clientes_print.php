...
$layout->blocks["top"][] = "pdf";$page_layouts["abonos_clientes_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		foreach(@$_REQUEST["mdelete"] as $ind)
		{
			...
			$keys["factura_servicio_id"]=refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys["factura_servicio_id"]=urldecode($arr[1]);
			$selected_recs[]=$keys;
		}
	}
...
$arr = array();
$arr['fName'] = "monto";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("monto");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "observaciones";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("observaciones");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "factura_servicio_id";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("factura_servicio_id");
$fieldsArr[] = $arr;
...
//	factura_servicio_id - 
			$record["factura_servicio_id_value"] = $pageObject->showDBValue("factura_servicio_id", $data, $keylink);
			$record["factura_servicio_id_class"] = $pageObject->fieldClass("factura_servicio_id");
//	fecha - Short Date
			$record["fecha_value"] = $pageObject->showDBValue("fecha", $data, $keylink);
			$record["fecha_class"] = $pageObject->fieldClass("fecha");
//	monto - Number
			$record["monto_value"] = $pageObject->showDBValue("monto", $data, $keylink);
			$record["monto_class"] = $pageObject->fieldClass("monto");
//	observaciones - 
			$record["observaciones_value"] = $pageObject->showDBValue("observaciones", $data, $keylink);
			$record["observaciones_class"] = $pageObject->fieldClass("observaciones");
			if($col<$colsonpage)
...
if($mastertable == "factura_consulta")
{
//	include proper masterprint.php code
	include("include/factura_consulta_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"abonos_clientes","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_factura_consulta";
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["factura_consulta_masterprint"] = $layout;
	...
	$layout = GetPageLayout("factura_consulta", 'masterprint');
	if($layout)
	{
...
$xt->assign("factura_servicio_id_fieldheadercolumn",true);
$xt->assign("factura_servicio_id_fieldheader",true);
$xt->assign("factura_servicio_id_fieldcolumn",true);
$xt->assign("factura_servicio_id_fieldfootercolumn",true);
...
$xt->assign("monto_fieldheadercolumn",true);
$xt->assign("monto_fieldheader",true);
$xt->assign("monto_fieldcolumn",true);
$xt->assign("monto_fieldfootercolumn",true);
$xt->assign("observaciones_fieldheadercolumn",true);
$xt->assign("observaciones_fieldheader",true);
$xt->assign("observaciones_fieldcolumn",true);
$xt->assign("observaciones_fieldfootercolumn",true);
