...
$layout->blocks["top"][] = "details";$page_layouts["solicitud_medicamento_edit"] = $layout;
...
$templatefile = "solicitud_medicamento_edit.htm";
...
//	processing fecha - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_fecha = $pageObject->getControl("fecha", $id);
		$control_fecha->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing responsable - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_responsable = $pageObject->getControl("responsable", $id);
		$control_responsable->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing responsable - end
//	processing medicamento_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_medicamento_id = $pageObject->getControl("medicamento_id", $id);
		$control_medicamento_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing medicamento_id - end
//	processing lote_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_lote_id = $pageObject->getControl("lote_id", $id);
		$control_lote_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: solicitud_medicamento_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: solicitud_medicamento_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["fecha"] = $evalues["fecha"];
	$data["consulta_id"] = $evalues["consulta_id"];
	$data["responsable"] = $evalues["responsable"];
	$data["medicamento_id"] = $evalues["medicamento_id"];
	$data["lote_id"] = $evalues["lote_id"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_label","<label for=\"".GetInputElementId("fecha", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("responsable_label","<label for=\"".GetInputElementId("responsable", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("medicamento_id_label","<label for=\"".GetInputElementId("medicamento_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("lote_id_label","<label for=\"".GetInputElementId("lote_id", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='solicitud_medicamento_view.php?".$viewlink."'\"");
