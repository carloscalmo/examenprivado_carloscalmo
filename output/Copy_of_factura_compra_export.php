...
$layout->blocks["top"][] = "export";$page_layouts["Copy_of_factura_compra_export"] = $layout;
...
$xt->display("Copy_of_factura_compra_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Copy_of_factura_compra.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_factura_compra.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_factura_compra.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["id1"] = $pageObject->showDBValue("id1", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_factura_compra.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["id1"] = $pageObject->getViewControl("id1")->showDBValue($row, "");
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Numero de Factura").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Proveedor").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id1").'</td>';	
