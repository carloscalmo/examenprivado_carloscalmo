...
$layout->blocks["top"][] = "details";$page_layouts["detalle_compra_edit"] = $layout;
...
$templatefile = "detalle_compra_edit.htm";
...
$keys["factura_compra"] = urldecode(postvalue("editid2"));
$savedKeys["factura_compra"] = urldecode(postvalue("editid2"));
...
//	processing producto - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_producto = $pageObject->getControl("producto", $id);
		$control_producto->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing producto - end
//	processing lote - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_lote = $pageObject->getControl("lote", $id);
		$control_lote->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing factura_compra - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_factura_compra = $pageObject->getControl("factura_compra", $id);
		$control_factura_compra->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_factura_compra->getWebValue()!==false)
			$keys["factura_compra"] = $control_factura_compra->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["factura_compra"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: detalle_compra_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: detalle_compra_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["producto"] = $evalues["producto"];
	$data["lote"] = $evalues["lote"];
	$data["cantidad"] = $evalues["cantidad"];
	$data["factura_compra"] = $evalues["factura_compra"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("producto_label","<label for=\"".GetInputElementId("producto", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("lote_label","<label for=\"".GetInputElementId("lote", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("factura_compra_label","<label for=\"".GetInputElementId("factura_compra", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("factura_compra", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='detalle_compra_view.php?".$viewlink."'\"");
