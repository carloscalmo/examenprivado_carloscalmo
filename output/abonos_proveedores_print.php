...
$layout->blocks["top"][] = "pdf";$page_layouts["abonos_proveedores_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		foreach(@$_REQUEST["mdelete"] as $ind)
		{
			...
			$keys["factura"]=refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
//	factura - 
			$record["factura_value"] = $pageObject->showDBValue("factura", $data, $keylink);
			$record["factura_class"] = $pageObject->fieldClass("factura");
//	fecha - Short Date
...
//	include proper masterprint.php code
	...
	$params = array("detailtable"=>"abonos_proveedores","keys"=>$masterkeys);
	$master = array();
...
$xt->assign("factura_fieldheadercolumn",true);
$xt->assign("factura_fieldheader",true);
$xt->assign("factura_fieldcolumn",true);
$xt->assign("factura_fieldfootercolumn",true);
