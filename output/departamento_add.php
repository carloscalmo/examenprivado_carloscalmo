...
add_nocache_headers();
include("include/departamento_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["departamento_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "departamento_inline_add.htm";
else
	$templatefile = "departamento_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: departamento_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'departamento_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'departamento_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: departamento_".$pageObject->getPageType().".php");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["municipio"]["masterkey1"] = $data["id"];	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "departamento";
		$options['firstTime'] = 1;
