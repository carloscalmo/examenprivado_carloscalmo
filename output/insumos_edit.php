...
$layout->blocks["top"][] = "details";$page_layouts["insumos_edit"] = $layout;
...
$keys["codigo"] = urldecode(postvalue("editid2"));
$savedKeys["codigo"] = urldecode(postvalue("editid2"));
...
//	processing contenido - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_contenido = $pageObject->getControl("contenido", $id);
		$control_contenido->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing contenido - end
//	processing componente - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_componente = $pageObject->getControl("componente", $id);
		$control_componente->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing presentacion - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_presentacion = $pageObject->getControl("presentacion", $id);
		$control_presentacion->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing presentacion - end
//	processing existencia_minima - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_existencia_minima = $pageObject->getControl("existencia_minima", $id);
		$control_existencia_minima->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing existencia_minima - end
//	processing existencia_actual - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_existencia_actual = $pageObject->getControl("existencia_actual", $id);
		$control_existencia_actual->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing existencia_actual - end
//	processing descripcion - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_descripcion = $pageObject->getControl("descripcion", $id);
		$control_descripcion->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["codigo"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: insumos_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["contenido"] = $evalues["contenido"];
	$data["componente"] = $evalues["componente"];
	$data["cantidad"] = $evalues["cantidad"];
	$data["presentacion"] = $evalues["presentacion"];
	$data["existencia_minima"] = $evalues["existencia_minima"];
	$data["existencia_actual"] = $evalues["existencia_actual"];
	$data["descripcion"] = $evalues["descripcion"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("contenido_label","<label for=\"".GetInputElementId("contenido", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("componente_label","<label for=\"".GetInputElementId("componente", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("presentacion_label","<label for=\"".GetInputElementId("presentacion", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("existencia_minima_label","<label for=\"".GetInputElementId("existencia_minima", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("existencia_actual_label","<label for=\"".GetInputElementId("existencia_actual", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("descripcion_label","<label for=\"".GetInputElementId("descripcion", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='insumos_view.php?".$viewlink."'\"");
