...
add_nocache_headers();
include("include/tipo_consulta_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["tipo_consulta_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tipo_consulta_inline_add.htm";
else
	$templatefile = "tipo_consulta_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tipo_consulta_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tipo_consulta_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tipo_consulta_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tipo_consulta_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tipo_consulta";
		$options['firstTime'] = 1;
