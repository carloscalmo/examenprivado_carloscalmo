...
$layout->blocks["top"][] = "details";$page_layouts["detalle_cirugia_edit"] = $layout;
...
$templatefile = "detalle_cirugia_edit.htm";
...
$keys["idtipo_cirugia"] = urldecode(postvalue("editid2"));
$savedKeys["idtipo_cirugia"] = urldecode(postvalue("editid2"));
...
//	processing idtipo_cirugia - begin
	...
	if($condition)
	{
		...
		$control_idtipo_cirugia->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
			...
			//	update key value
		if($control_idtipo_cirugia->getWebValue()!==false)
			$keys["idtipo_cirugia"] = $control_idtipo_cirugia->getWebValue();
	}
//	processing idtipo_cirugia - end
//	processing insumos_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_insumos_id = $pageObject->getControl("insumos_id", $id);
		$control_insumos_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["idtipo_cirugia"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: detalle_cirugia_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: detalle_cirugia_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["insumos_id"] = $evalues["insumos_id"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("insumos_id_label","<label for=\"".GetInputElementId("insumos_id", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("idtipo_cirugia", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='detalle_cirugia_view.php?".$viewlink."'\"");
