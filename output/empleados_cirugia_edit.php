...
$layout->blocks["top"][] = "details";$page_layouts["empleados_cirugia_edit"] = $layout;
...
$templatefile = "empleados_cirugia_edit.htm";
...
$keys["cirugia_id"] = urldecode(postvalue("editid2"));
$savedKeys["cirugia_id"] = urldecode(postvalue("editid2"));
...
//	processing cirugia_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_cirugia_id = $pageObject->getControl("cirugia_id", $id);
		$control_cirugia_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_cirugia_id->getWebValue()!==false)
			$keys["cirugia_id"] = $control_cirugia_id->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["cirugia_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: empleados_cirugia_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: empleados_cirugia_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["cirugia_id"] = $evalues["cirugia_id"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("cirugia_id_label","<label for=\"".GetInputElementId("cirugia_id", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("id", $data)));
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("cirugia_id", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='empleados_cirugia_view.php?".$viewlink."'\"");
