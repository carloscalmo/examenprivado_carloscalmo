...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["persona_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("municipio_id", $strTableName, false)."=".make_db_value("municipio_id",$_SESSION[$strTableName."_masterkey1"]);
}
if($mastertable == "genero")
{
	$where = "";
		$where .= GetFullFieldName("genero_id", $strTableName, false)."=".make_db_value("genero_id",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	fecha_nacimiento - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("fecha_nacimiento", $data, $keylink);
			$row["fecha_nacimiento_value"] = $value;
	//	cui - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("cui", $data, $keylink);
			$row["cui_value"] = $value;
	//	direccion - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template("persona_detailspreview.htm");
