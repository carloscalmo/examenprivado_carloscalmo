...
$layout->blocks["top"][] = "pdf";$page_layouts["detalle_compra_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		foreach(@$_REQUEST["mdelete"] as $ind)
		{
			...
			$keys["factura_compra"]=refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys["factura_compra"]=urldecode($arr[1]);
			$selected_recs[]=$keys;
		}
	}
...
$arr = array();
$arr['fName'] = "producto";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("producto");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "lote";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("lote");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "cantidad";
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "factura_compra";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("factura_compra");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "costo";
...
//	producto - 
			$record["producto_value"] = $pageObject->showDBValue("producto", $data, $keylink);
			$record["producto_class"] = $pageObject->fieldClass("producto");
//	lote - 
			$record["lote_value"] = $pageObject->showDBValue("lote", $data, $keylink);
			$record["lote_class"] = $pageObject->fieldClass("lote");
//	cantidad - Number
			$record["cantidad_value"] = $pageObject->showDBValue("cantidad", $data, $keylink);
			$record["cantidad_class"] = $pageObject->fieldClass("cantidad");
//	factura_compra - 
			$record["factura_compra_value"] = $pageObject->showDBValue("factura_compra", $data, $keylink);
			$record["factura_compra_class"] = $pageObject->fieldClass("factura_compra");
//	costo - Number
...
//	include proper masterprint.php code
	...
	$params = array("detailtable"=>"detalle_compra","keys"=>$masterkeys);
	$master = array();
...
if($mastertable == "factura_compra")
{
//	include proper masterprint.php code
	include("include/factura_compra_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"detalle_compra","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_factura_compra";
	$master["params"] = $params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$layout = new TLayout("masterprint","Rounded1BlueWave","MobileBlueWave");
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["factura_compra_masterprint"] = $layout;
	...
	$layout = GetPageLayout("factura_compra", 'masterprint');
	if($layout)
	{
		$rtl = $pageObject->xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
		$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
{
//	include proper masterprint.php code
	include("include/lotes_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"detalle_compra","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_lotes";
	$master["params"] = $params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$layout = new TLayout("masterprint","Rounded1BlueWave","MobileBlueWave");
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["lotes_masterprint"] = $layout;
	...
	$layout = GetPageLayout("lotes", 'masterprint');
	if($layout)
	{
		$rtl = $pageObject->xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
		$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
$xt->assign("producto_fieldheadercolumn",true);
$xt->assign("producto_fieldheader",true);
$xt->assign("producto_fieldcolumn",true);
$xt->assign("producto_fieldfootercolumn",true);
$xt->assign("lote_fieldheadercolumn",true);
$xt->assign("lote_fieldheader",true);
$xt->assign("lote_fieldcolumn",true);
$xt->assign("lote_fieldfootercolumn",true);
...
$xt->assign("factura_compra_fieldheadercolumn",true);
$xt->assign("factura_compra_fieldheader",true);
$xt->assign("factura_compra_fieldcolumn",true);
$xt->assign("factura_compra_fieldfootercolumn",true);
