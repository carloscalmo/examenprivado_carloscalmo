...
add_nocache_headers();
include("include/Copy_of_lotes_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["Copy_of_lotes_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Copy_of_lotes_inline_add.htm";
else
	$templatefile = "Copy_of_lotes_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Copy_of_lotes_add.php');
...
//	processing codigo_insumo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_codigo_insumo = $pageObject->getControl("codigo_insumo", $id);
		$control_codigo_insumo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing codigo_insumo - end
...
//	processing codigo_insumo - start
			...
			if($inlineAddOption)
			{
				$control_codigo_insumo->afterSuccessfulSave();
			}
//	processing codigo_insumo - end
//	processing proveedor - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Copy_of_lotes_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Copy_of_lotes_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Copy_of_lotes_".$pageObject->getPageType().".php");
...
//	codigo_insumo
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("codigo_insumo", $data, $keylink);
		$showValues["codigo_insumo"] = $value;
		$showFields[] = "codigo_insumo";
	}	
//	proveedor
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showRawValues["codigo_insumo"] = substr($data["codigo_insumo"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Copy of lotes";
		$options['firstTime'] = 1;
