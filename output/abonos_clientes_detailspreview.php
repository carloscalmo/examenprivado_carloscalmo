...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["abonos_clientes_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("factura_servicio_id", $strTableName, false)."=".make_db_value("factura_servicio_id",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	monto - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("monto", $data, $keylink);
			$row["monto_value"] = $value;
	//	observaciones - 
			$viewContainer->recId = $recordsCounter;
	...
	//	factura_servicio_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("factura_servicio_id", $data, $keylink);
			$row["factura_servicio_id_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("abonos_clientes_detailspreview.htm");
