...
$layout->blocks["top"][] = "pdf";$page_layouts["tipo_ambiente_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["idtipo_ambiente"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
	...
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<2)
				continue;
			$keys=array();
			$keys["idtipo_ambiente"]=urldecode($arr[0]);
...
//	idtipo_ambiente - 
			$record["idtipo_ambiente_value"] = $pageObject->showDBValue("idtipo_ambiente", $data, $keylink);
			$record["idtipo_ambiente_class"] = $pageObject->fieldClass("idtipo_ambiente");
//	nombre - 
...
$xt->assign("idtipo_ambiente_fieldheadercolumn",true);
$xt->assign("idtipo_ambiente_fieldheader",true);
$xt->assign("idtipo_ambiente_fieldcolumn",true);
$xt->assign("idtipo_ambiente_fieldfootercolumn",true);
