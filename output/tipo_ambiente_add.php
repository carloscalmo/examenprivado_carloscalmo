...
$layout->blocks["top"][] = "details";$page_layouts["tipo_ambiente_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tipo_ambiente_inline_add.htm";
else
	$templatefile = "tipo_ambiente_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tipo_ambiente_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tipo_ambiente_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tipo_ambiente_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tipo_ambiente_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["idtipo_ambiente"]=postvalue("copyid1");
		$copykeys["nombre"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["idtipo_ambiente"]=postvalue("editid1");
