...
$layout->blocks["top"][] = "details";$page_layouts["usuarios_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "usuarios_inline_add.htm";
else
	$templatefile = "usuarios_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing Rol - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Rol = $pageObject->getControl("Rol", $id);
		$control_Rol->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Rol - end
...
//	processing contraseña - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_contrase_a = $pageObject->getControl("contraseña", $id);
		$control_contrase_a->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing contraseña - end
//	processing empleado_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_empleado_id = $pageObject->getControl("empleado_id", $id);
		$control_empleado_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing empleado_id - end
//	processing fecha_inicio - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_fecha_inicio = $pageObject->getControl("fecha_inicio", $id);
		$control_fecha_inicio->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing fecha_inicio - end
//	processing fecha_fin - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_fecha_fin = $pageObject->getControl("fecha_fin", $id);
		$control_fecha_fin->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing fecha_fin - end
//	processing baja - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_baja = $pageObject->getControl("baja", $id);
		$control_baja->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing baja - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
	{
		...
		if($avalues["empleado_id"]==""){
			$avalues["empleado_id"] = prepare_for_db("empleado_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'usuarios_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'usuarios_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: usuarios_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["empleado_id"]=postvalue("copyid3");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["nombre"]=postvalue("editid2");
		$copykeys["empleado_id"]=postvalue("editid3");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["empleado_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["contraseña"]=@$avalues["contraseña"];
	$defvalues["fecha_inicio"]=@$avalues["fecha_inicio"];
	$defvalues["fecha_fin"]=@$avalues["fecha_fin"];
	$defvalues["baja"]=@$avalues["baja"];
	$defvalues["empleado_id"]=@$avalues["empleado_id"];
	$defvalues["Rol"]=@$avalues["Rol"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("contrase_a_label","<label for=\"".GetInputElementId("contraseña", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_inicio_label","<label for=\"".GetInputElementId("fecha_inicio", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_fin_label","<label for=\"".GetInputElementId("fecha_fin", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("baja_label","<label for=\"".GetInputElementId("baja", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("empleado_id_label","<label for=\"".GetInputElementId("empleado_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Rol_label","<label for=\"".GetInputElementId("Rol", $id, PAGE_ADD)."\">","</label>");
