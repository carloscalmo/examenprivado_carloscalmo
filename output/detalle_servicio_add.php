...
add_nocache_headers();
include("include/detalle_servicio_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["detalle_servicio_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "detalle_servicio_inline_add.htm";
else
	$templatefile = "detalle_servicio_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: detalle_servicio_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'detalle_servicio_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'detalle_servicio_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: detalle_servicio_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["servicio_id"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["servicio_id"]=postvalue("editid2");
	}
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink.="&key2=".htmlspecialchars(rawurlencode(@$data["servicio_id"]));
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "detalle_servicio";
		$options['firstTime'] = 1;
