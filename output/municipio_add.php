...
add_nocache_headers();
include("include/municipio_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["municipio_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "municipio_inline_add.htm";
else
	$templatefile = "municipio_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: municipio_add.php');
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="departamento")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["departamento"]==""){
			$avalues["departamento"] = prepare_for_db("departamento",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'municipio_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'municipio_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: municipio_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="departamento")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["departamento"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["departamento"]=@$avalues["departamento"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("departamento"))
		$xt->assign("departamento_fieldblock",true);
	else
		$xt->assign("departamento_tabfieldblock",true);
	$xt->assign("departamento_label",true);
	if(isEnableSection508())
		$xt->assign_section("departamento_label","<label for=\"".GetInputElementId("departamento", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["proveedores"]["masterkey1"] = $data["id"];	
	$showDetailKeys["persona"]["masterkey1"] = $data["id"];	
