...
$layout->blocks["top"][] = "details";$page_layouts["tipo_cirugia_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tipo_cirugia_inline_add.htm";
else
	$templatefile = "tipo_cirugia_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tipo_cirugia_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tipo_cirugia_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tipo_cirugia_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tipo_cirugia_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["idtipo_cirugia"]=postvalue("copyid1");
		$copykeys["nombre"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["idtipo_cirugia"]=postvalue("editid1");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["cirugia"]["masterkey1"] = $data["idtipo_cirugia"];	
	$showDetailKeys["detalle_cirugia"]["masterkey1"] = $data["idtipo_cirugia"];	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tipo_cirugia";
		$options['firstTime'] = 1;
