...
$layout->blocks["top"][] = "details";$page_layouts["tipo_ambiente_edit"] = $layout;
...
$templatefile = "tipo_ambiente_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["idtipo_ambiente"] = urldecode(postvalue("editid1"));
$savedKeys["idtipo_ambiente"] = urldecode(postvalue("editid1"));
...
//	processing idtipo_ambiente - begin
	...
	if($condition)
	{
		...
		$control_idtipo_ambiente->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_idtipo_ambiente->getWebValue()!==false)
			$keys["idtipo_ambiente"] = $control_idtipo_ambiente->getWebValue();
		}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idtipo_ambiente"])."&";
		$keyGetQ.="editid2=".rawurldecode($keys["nombre"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: tipo_ambiente_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: tipo_ambiente_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["idtipo_ambiente"] = $evalues["idtipo_ambiente"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("idtipo_ambiente_label","<label for=\"".GetInputElementId("idtipo_ambiente", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("idtipo_ambiente", $data)));
...
//	detail tables
	$showDetailKeys["ambiente"]["masterkey1"] = $data["idtipo_ambiente"];		
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tipo_ambiente";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tipo_ambiente_view.php?".$viewlink."'\"");
