...
$layout->blocks["top"][] = "details";$page_layouts["abonos_clientes_edit"] = $layout;
...
$templatefile = "abonos_clientes_edit.htm";
...
$keys["factura_servicio_id"] = urldecode(postvalue("editid2"));
$savedKeys["factura_servicio_id"] = urldecode(postvalue("editid2"));
...
//	processing observaciones - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_observaciones = $pageObject->getControl("observaciones", $id);
		$control_observaciones->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing observaciones - end
//	processing factura_servicio_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_factura_servicio_id = $pageObject->getControl("factura_servicio_id", $id);
		$control_factura_servicio_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_factura_servicio_id->getWebValue()!==false)
			$keys["factura_servicio_id"] = $control_factura_servicio_id->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["factura_servicio_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: abonos_clientes_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: abonos_clientes_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["factura_servicio_id"] = $evalues["factura_servicio_id"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("observaciones_label","<label for=\"".GetInputElementId("observaciones", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("factura_servicio_id_label","<label for=\"".GetInputElementId("factura_servicio_id", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("id", $data)));
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("factura_servicio_id", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='abonos_clientes_view.php?".$viewlink."'\"");
