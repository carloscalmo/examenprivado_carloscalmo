...
class RightsPage extends ListPage  
{
	...
	function fillGroupsArr(){
		//	select groups list
		$this->groupsArr[] = array(-1, "<"."Administrador".">");
		$this->groupsArr[] = array(-2, "<"."Por defecto".">");
		$this->groupsArr[] = array(-3, "<"."Invitado".">");
	...
	function getRights() 
	{
		$trs = db_query("select `GroupID`,`TableName`,`AccessMask` from `permisosugrights` order by `GroupID`", $this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
