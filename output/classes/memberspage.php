...
{	
	...
	*/
	...
	{	
		...
		$userheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby=Anombre\"";
		
		//for display name 
		$displayuserheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby="."Anombre\"";
		...
		if ($this->sortByGroup === false && $this->sortField != ""){
			if ($this->sortField == "nombre"){
				...
				if($this->sortOrder == "A") {
					...
					$displayuserheaderlink_attrs.="Dnombre";
				} else {
					if($this->is508)
					...
					$displayuserheaderlink_attrs.="Anombre";
				}
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `permisosugmembers` where UserName=".db_prepare_string($user);
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `permisosugmembers` (UserName,GroupID) values (".db_prepare_string(htmlspecialchars($user)).",".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `permisosugmembers` (UserName,GroupID) select ".db_prepare_string(htmlspecialchars($user)).",GroupID from `permisosuggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			foreach($this->groups as $idx => $g)
			{
				...
				if($_SESSION["UserID"] != $data["nombre"] || $g[0] != -1)
				{
			...
			//$row["begin"] = "<input type=hidden name=\"username[]\" value=\"".htmlspecialchars($data["nombre"])."\">";
			
			$row["username"] = htmlspecialchars($data["nombre"]);
			$row["displayusername"] = htmlspecialchars($data["nombre"]); //htmlspecialchars($data["nombre"]);
			$row["user"] = $data["nombre"];
			$usercheckbox_attrs.=" name=\"user_".count($this->users)
				."\" id=\"".count($this->users)."\"";
			
			$this->users[] = htmlspecialchars($data["nombre"]);
	...
	{
		//	select members list		
		$trs = db_query("select `UserName`,`GroupID` from `permisosugmembers` order by `UserName`,`GroupID`",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select `GroupID`,`Label` from `permisosuggroups` order by `Label`",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
	...
	function setSessionVariables() 
	{
		...
		if (substr($_SESSION[$this->sessionPrefix."_orderby"],1) == "nombre" || substr($_SESSION[$this->sessionPrefix."_orderby"],1) =="nombre"){
