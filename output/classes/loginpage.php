...
class LoginPage extends RunnerPage 
{
	...
	*/
	function LogIn($pUsername,$pPassword){
		...
		$fieldList = "";
		$lSet = new ProjectSettings("usuarios", PAGE_LIST);
		if($lSet->GetTableData(".sqlquery"))
			$fieldList = $lSet->GetTableData(".sqlquery")->toSql();
		if($fieldList)
		{
			if(!$this->pSet->isCaseInsensitiveUsername()) {
				$where = GetFullFieldName($cUserNameField,"usuarios",false).
				   "=".$strUsername." and ".GetFullFieldName($cPasswordField,"usuarios",false)."=".$strPassword;
   			} else {
				$where = db_upper(getFullFieldName($cUserNameField,"usuarios",false)).
					   "=".$this->pSet->getCaseSensitiveUsername($strUsername)." and ".GetFullFieldName($cPasswordField,"usuarios",false).
					   "=".$strPassword;
			}		   
		...
		{
			$strSQL = "select * from `usuarios` where ".AddFieldWrappers($cUserNameField)."=".$strUsername." and ".AddFieldWrappers($cPasswordField)."=".$strPassword;
					}
