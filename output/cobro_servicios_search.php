...
include("include/cobro_servicios_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["cobro_servicios_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "cobro_servicios_search.htm";
...
$params['shortTableName'] = 'cobro_servicios';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("'Servicio Contratado'"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("'Servicio Contratado'")] = GetTableURL($pageObject->pSet->getLookupTable("'Servicio Contratado'"));
	
	$pageObject->fillFieldToolTips("'Servicio Contratado'");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("'Servicio Contratado'");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "'Servicio Contratado'";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("_Servicio_Contratado__label","<label for=\"".GetInputElementId("'Servicio Contratado'", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("_Servicio_Contratado__label", true);
	
	$xt->assign("_Servicio_Contratado__fieldblock", true);
	$xt->assignbyref("_Servicio_Contratado__editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("_Servicio_Contratado__notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("_Servicio_Contratado__editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype__Servicio_Contratado_", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("'Servicio Contratado'");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"'Servicio Contratado'", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"'Servicio Contratado'", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
