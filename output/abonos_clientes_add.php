...
add_nocache_headers();
include("include/abonos_clientes_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["abonos_clientes_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "abonos_clientes_inline_add.htm";
else
	$templatefile = "abonos_clientes_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: abonos_clientes_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing factura_servicio_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_factura_servicio_id = $pageObject->getControl("factura_servicio_id", $id);
		$control_factura_servicio_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing factura_servicio_id - end
...
//	processing observaciones - start
	...
	if($inlineAddOption)
	{
		$control_observaciones = $pageObject->getControl("observaciones", $id);
		$control_observaciones->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing observaciones - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="factura_consulta")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["factura_servicio_id"]==""){
			$avalues["factura_servicio_id"] = prepare_for_db("factura_servicio_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing factura_servicio_id - start
			...
			if($inlineAddOption)
			{
				$control_factura_servicio_id->afterSuccessfulSave();
			}
//	processing factura_servicio_id - end
//	processing fecha - start
			$inlineAddOption = true;
...
//	processing observaciones - start
			...
			if($inlineAddOption)
			{
				$control_observaciones->afterSuccessfulSave();
			}
//	processing observaciones - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'abonos_clientes_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'abonos_clientes_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: abonos_clientes_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["factura_servicio_id"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["factura_servicio_id"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="factura_consulta")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["factura_servicio_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["observaciones"]=@$avalues["observaciones"];
	$defvalues["factura_servicio_id"]=@$avalues["factura_servicio_id"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("observaciones"))
		$xt->assign("observaciones_fieldblock",true);
	else
		$xt->assign("observaciones_tabfieldblock",true);
	$xt->assign("observaciones_label",true);
	if(isEnableSection508())
		$xt->assign_section("observaciones_label","<label for=\"".GetInputElementId("observaciones", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("factura_servicio_id"))
		$xt->assign("factura_servicio_id_fieldblock",true);
	else
		$xt->assign("factura_servicio_id_tabfieldblock",true);
	$xt->assign("factura_servicio_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("factura_servicio_id_label","<label for=\"".GetInputElementId("factura_servicio_id", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$keylink.="&key2=".htmlspecialchars(rawurlencode(@$data["factura_servicio_id"]));
...
//	observaciones
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("observaciones", $data, $keylink);
		$showValues["observaciones"] = $value;
...
//	factura_servicio_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("factura_servicio_id", $data, $keylink);
		$showValues["factura_servicio_id"] = $value;
		$showFields[] = "factura_servicio_id";
	}	
		...
		$showRawValues["observaciones"] = substr($data["observaciones"],0,100);
		$showRawValues["factura_servicio_id"] = substr($data["factura_servicio_id"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "abonos_clientes";
		$options['firstTime'] = 1;
