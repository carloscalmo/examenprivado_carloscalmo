...
$layout->blocks["top"][] = "details";$page_layouts["insumos_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "insumos_inline_add.htm";
else
	$templatefile = "insumos_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: insumos_add.php');
...
//	processing componente - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_componente = $pageObject->getControl("componente", $id);
		$control_componente->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing componente - end
//	processing contenido - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_contenido = $pageObject->getControl("contenido", $id);
		$control_contenido->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing contenido - end
//	processing presentacion - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_presentacion = $pageObject->getControl("presentacion", $id);
		$control_presentacion->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing presentacion - end
...
//	processing existencia_actual - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_existencia_actual = $pageObject->getControl("existencia_actual", $id);
		$control_existencia_actual->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing existencia_actual - end
//	processing existencia_minima - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_existencia_minima = $pageObject->getControl("existencia_minima", $id);
		$control_existencia_minima->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing existencia_minima - end
//	processing descripcion - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_descripcion = $pageObject->getControl("descripcion", $id);
		$control_descripcion->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing descripcion - end
...
//	processing componente - start
			...
			if($inlineAddOption)
			{
				$control_componente->afterSuccessfulSave();
			}
//	processing componente - end
//	processing contenido - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_contenido->afterSuccessfulSave();
			}
//	processing contenido - end
//	processing presentacion - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_presentacion->afterSuccessfulSave();
			}
//	processing presentacion - end
//	processing cantidad - start
			$inlineAddOption = true;
...
//	processing existencia_actual - start
			...
			if($inlineAddOption)
			{
				$control_existencia_actual->afterSuccessfulSave();
			}
//	processing existencia_actual - end
//	processing existencia_minima - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_existencia_minima->afterSuccessfulSave();
			}
//	processing existencia_minima - end
//	processing descripcion - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_descripcion->afterSuccessfulSave();
			}
//	processing descripcion - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'insumos_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'insumos_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: insumos_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["codigo"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["codigo"]=postvalue("editid2");
	}
...
if($readavalues)
{
	...
	$defvalues["contenido"]=@$avalues["contenido"];
	$defvalues["componente"]=@$avalues["componente"];
	$defvalues["cantidad"]=@$avalues["cantidad"];
	$defvalues["presentacion"]=@$avalues["presentacion"];
	$defvalues["existencia_minima"]=@$avalues["existencia_minima"];
	$defvalues["existencia_actual"]=@$avalues["existencia_actual"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("contenido_label","<label for=\"".GetInputElementId("contenido", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("componente_label","<label for=\"".GetInputElementId("componente", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("presentacion_label","<label for=\"".GetInputElementId("presentacion", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("existencia_minima_label","<label for=\"".GetInputElementId("existencia_minima", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("existencia_actual_label","<label for=\"".GetInputElementId("existencia_actual", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("descripcion_label","<label for=\"".GetInputElementId("descripcion", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["detalle_servicio"]["masterkey1"] = $data["id"];	
	$showDetailKeys["detalle_cirugia"]["masterkey1"] = $data["id"];	
	$showDetailKeys["detalle_compra"]["masterkey1"] = $data["id"];	
	$showDetailKeys["lotes"]["masterkey1"] = $data["id"];	
...
//	contenido
	...
	{	
		...
		$showValues["contenido"] = $value;
		$showFields[] = "contenido";
	}	
//	componente
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showValues["componente"] = $value;
		$showFields[] = "componente";
	}	
//	cantidad
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	presentacion
	...
	{	
		...
		$showValues["presentacion"] = $value;
		$showFields[] = "presentacion";
	}	
//	existencia_minima
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showValues["existencia_minima"] = $value;
		$showFields[] = "existencia_minima";
	}	
//	existencia_actual
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showValues["existencia_actual"] = $value;
		$showFields[] = "existencia_actual";
	}	
//	descripcion
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showRawValues["contenido"] = substr($data["contenido"],0,100);
		$showRawValues["componente"] = substr($data["componente"],0,100);
		$showRawValues["cantidad"] = substr($data["cantidad"],0,100);
		$showRawValues["presentacion"] = substr($data["presentacion"],0,100);
		$showRawValues["existencia_minima"] = substr($data["existencia_minima"],0,100);
		$showRawValues["existencia_actual"] = substr($data["existencia_actual"],0,100);
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="detalle_compra" && postvalue("field")=="producto") 
	{
		$lookupFieldName = "producto";
		$lookupTable = "insumos";
		$linkFieldName = "id";
		$dispfield = "concat(codigo,' ',nombre)";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "detalle_compra", "producto");
			...
			$LookupSQL .= GetFullFieldName("id", $strTableName, true);
				...
				$LookupSQL .= ",concat(codigo,' ',nombre)";
		}
	}
