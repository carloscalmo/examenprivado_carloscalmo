...
include_once("include/empleado_settings.php");
...
$layout->blocks["top"][] = "toplinks";$page_layouts["persona_list"] = $layout;
...
{	
	...
	//determine which field should be used to select values
			$params["lookupSelectField"] = "nombre";
								}
...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["municipio_masterlist"] = $layout;
...
$viewControls = new ViewControlsContainer(new ProjectSettings("municipio", PAGE_LIST), PAGE_LIST);
...
$layout->skins["mastergrid"] = "grid";
$layout->blocks["bare"][] = "mastergrid";$page_layouts["genero_masterlist"] = $layout;

$layout = GetPageLayout("genero", 'masterlist');
if($layout)
{
	$rtl = $xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
	$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
include_once getabspath('classes/controls/ViewControlsContainer.php');
$viewControls = new ViewControlsContainer(new ProjectSettings("genero", PAGE_LIST), PAGE_LIST);
$viewControls->addControlsJSAndCSS();
$pageObject->includes_js = array_merge($pageObject->includes_js, $viewControls->includes_js);
$pageObject->includes_jsreq = array_merge($pageObject->includes_jsreq, $viewControls->includes_jsreq);
$pageObject->includes_css = array_merge($pageObject->includes_css, $viewControls->includes_css);
