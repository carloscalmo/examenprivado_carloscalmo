...
$layout->blocks["top"][] = "details";$page_layouts["Copy_of_lotes_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Copy_of_lotes_inline_edit.htm" : "Copy_of_lotes_edit.htm";
...
//	processing codigo_insumo - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_codigo_insumo = $pageObject->getControl("codigo_insumo", $id);
		$control_codigo_insumo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Copy_of_lotes_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Copy_of_lotes_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["codigo_insumo"] = $evalues["codigo_insumo"];
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Copy_of_lotes_view.php?".$viewlink."'\"");
