<?php
// $Header: /cvsrepos/html2ps/css.parse.inc.php,v 1.1 2008/07/01 12:30:26 sergey Exp $
...
//		XLINESOFT-added block begin
//                         ":not(selector)"  | // :not pseudoclass
//		XLINESOFT-added block end
//
//
...
//	begin XLINESOFT
  ...
  if (preg_match("/^(\S+):not\(([#\.\s\w_-]+)\)$/", $raw_selector, $matches)) {
    return array(SELECTOR_SEQUENCE, array(parse_css_selector($matches[1]), 
		array( SELECTOR_NOT, parse_css_selector($matches[2]))));
  };
  // single :not(<selector>) 
  if (preg_match("/^:not\(([#\.\s\w_-]+)\)$/", $raw_selector, $matches)) {
    return array( SELECTOR_NOT, parse_css_selector($matches[1]));
  };
//	end XLINESOFT
