...
$layout->blocks["top"][] = "details";$page_layouts["solicitud_medicamento_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "solicitud_medicamento_inline_add.htm";
else
	$templatefile = "solicitud_medicamento_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: solicitud_medicamento_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing fecha - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_fecha = $pageObject->getControl("fecha", $id);
		$control_fecha->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing fecha - end
//	processing consulta_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_consulta_id = $pageObject->getControl("consulta_id", $id);
		$control_consulta_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing consulta_id - end
//	processing responsable - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_responsable = $pageObject->getControl("responsable", $id);
		$control_responsable->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing responsable - end
//	processing medicamento_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_medicamento_id = $pageObject->getControl("medicamento_id", $id);
		$control_medicamento_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing medicamento_id - end
//	processing lote_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_lote_id = $pageObject->getControl("lote_id", $id);
		$control_lote_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing lote_id - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="consulta")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["consulta_id"]==""){
			$avalues["consulta_id"] = prepare_for_db("consulta_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["responsable"]==""){
			$avalues["responsable"] = prepare_for_db("responsable",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="insumos")
	{
		...
		if($avalues["medicamento_id"]==""){
			$avalues["medicamento_id"] = prepare_for_db("medicamento_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="lotes")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["lote_id"]==""){
			$avalues["lote_id"] = prepare_for_db("lote_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing fecha - start
			...
			if($inlineAddOption)
			{
				$control_fecha->afterSuccessfulSave();
			}
//	processing fecha - end
//	processing consulta_id - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_consulta_id->afterSuccessfulSave();
			}
//	processing consulta_id - end
//	processing responsable - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_responsable->afterSuccessfulSave();
			}
//	processing responsable - end
//	processing medicamento_id - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_medicamento_id->afterSuccessfulSave();
			}
//	processing medicamento_id - end
//	processing lote_id - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_lote_id->afterSuccessfulSave();
			}
//	processing lote_id - end
//	processing cantidad - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'solicitud_medicamento_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'solicitud_medicamento_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: solicitud_medicamento_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="consulta")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["consulta_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["responsable"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="insumos")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["medicamento_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="lotes")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["lote_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["fecha"]=@$avalues["fecha"];
	$defvalues["consulta_id"]=@$avalues["consulta_id"];
	$defvalues["responsable"]=@$avalues["responsable"];
	$defvalues["medicamento_id"]=@$avalues["medicamento_id"];
	$defvalues["lote_id"]=@$avalues["lote_id"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_label","<label for=\"".GetInputElementId("fecha", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("consulta_id"))
		$xt->assign("consulta_id_fieldblock",true);
	else
		$xt->assign("consulta_id_tabfieldblock",true);
	$xt->assign("consulta_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("consulta_id_label","<label for=\"".GetInputElementId("consulta_id", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("responsable"))
		$xt->assign("responsable_fieldblock",true);
	else
		$xt->assign("responsable_tabfieldblock",true);
	$xt->assign("responsable_label",true);
	if(isEnableSection508())
		$xt->assign_section("responsable_label","<label for=\"".GetInputElementId("responsable", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("medicamento_id"))
		$xt->assign("medicamento_id_fieldblock",true);
	else
		$xt->assign("medicamento_id_tabfieldblock",true);
	$xt->assign("medicamento_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("medicamento_id_label","<label for=\"".GetInputElementId("medicamento_id", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("lote_id"))
		$xt->assign("lote_id_fieldblock",true);
	else
		$xt->assign("lote_id_tabfieldblock",true);
	$xt->assign("lote_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("lote_id_label","<label for=\"".GetInputElementId("lote_id", $id, PAGE_ADD)."\">","</label>");
...
//	consulta_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("consulta_id", $data, $keylink);
		$showValues["consulta_id"] = $value;
		$showFields[] = "consulta_id";
	}	
//	responsable
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("responsable", $data, $keylink);
		$showValues["responsable"] = $value;
		$showFields[] = "responsable";
	}	
//	medicamento_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("medicamento_id", $data, $keylink);
		$showValues["medicamento_id"] = $value;
		$showFields[] = "medicamento_id";
	}	
//	lote_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("lote_id", $data, $keylink);
		$showValues["lote_id"] = $value;
		$showFields[] = "lote_id";
	}	
//	cantidad
	$display = false;
	...
	{	
		...
		$showRawValues["fecha"] = substr($data["fecha"],0,100);
		$showRawValues["consulta_id"] = substr($data["consulta_id"],0,100);
		$showRawValues["responsable"] = substr($data["responsable"],0,100);
		$showRawValues["medicamento_id"] = substr($data["medicamento_id"],0,100);
		$showRawValues["lote_id"] = substr($data["lote_id"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "solicitud_medicamento";
		$options['firstTime'] = 1;
