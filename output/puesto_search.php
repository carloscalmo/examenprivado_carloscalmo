...
$layout->blocks["top"][] = "search";$page_layouts["puesto_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "puesto_search.htm";
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("salario"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("salario")] = GetTableURL($pageObject->pSet->getLookupTable("salario"));
	
	$pageObject->fillFieldToolTips("salario");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("salario");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "salario";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("salario_label","<label for=\"".GetInputElementId("salario", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("salario_label", true);
	
	$xt->assign("salario_fieldblock", true);
	$xt->assignbyref("salario_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("salario_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("salario_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_salario", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("salario");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"salario", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"salario", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
