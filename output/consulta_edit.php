...
$layout->blocks["top"][] = "details";$page_layouts["consulta_edit"] = $layout;
...
$templatefile = "consulta_edit.htm";
...
$keys["paciente"] = urldecode(postvalue("editid2"));
$savedKeys["paciente"] = urldecode(postvalue("editid2"));
...
//	processing paciente - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_paciente = $pageObject->getControl("paciente", $id);
		$control_paciente->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_paciente->getWebValue()!==false)
			$keys["paciente"] = $control_paciente->getWebValue();
	}
//	processing paciente - end
//	processing estado_retirada - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_estado_retirada = $pageObject->getControl("estado_retirada", $id);
		$control_estado_retirada->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing estado_retirada - end
//	processing medico_encargado - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_medico_encargado = $pageObject->getControl("medico_encargado", $id);
		$control_medico_encargado->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing medico_encargado - end
//	processing diagnostico_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_diagnostico_id = $pageObject->getControl("diagnostico_id", $id);
		$control_diagnostico_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing diagnostico_id - end
//	processing tipo_consulta_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tipo_consulta_id = $pageObject->getControl("tipo_consulta_id", $id);
		$control_tipo_consulta_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["paciente"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: consulta_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["paciente"] = $evalues["paciente"];
	$data["estado_retirada"] = $evalues["estado_retirada"];
	$data["medico_encargado"] = $evalues["medico_encargado"];
	$data["diagnostico_id"] = $evalues["diagnostico_id"];
	$data["tipo_consulta_id"] = $evalues["tipo_consulta_id"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("paciente_label","<label for=\"".GetInputElementId("paciente", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("estado_retirada_label","<label for=\"".GetInputElementId("estado_retirada", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("medico_encargado_label","<label for=\"".GetInputElementId("medico_encargado", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("diagnostico_id_label","<label for=\"".GetInputElementId("diagnostico_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tipo_consulta_id_label","<label for=\"".GetInputElementId("tipo_consulta_id", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='consulta_view.php?".$viewlink."'\"");
