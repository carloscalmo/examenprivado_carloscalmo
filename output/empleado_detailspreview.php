...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["empleado_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("persona_id", $strTableName, false)."=".make_db_value("persona_id",$_SESSION[$strTableName."_masterkey1"]);
}
if($mastertable == "puesto")
{
	$where = "";
		$where .= GetFullFieldName("puesto_id", $strTableName, false)."=".make_db_value("puesto_id",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	profesion - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("profesion", $data, $keylink);
			$row["profesion_value"] = $value;
	//	puesto_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("puesto_id", $data, $keylink);
			$row["puesto_id_value"] = $value;
	//	persona_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("persona_id", $data, $keylink);
			$row["persona_id_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("empleado_detailspreview.htm");
