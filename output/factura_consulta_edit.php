...
$layout->blocks["top"][] = "details";$page_layouts["factura_consulta_edit"] = $layout;
...
$templatefile = "factura_consulta_edit.htm";
...
//	processing numero_factura - begin
	$condition = 1;

	if($condition)
	{
		$control_numero_factura = $pageObject->getControl("numero_factura", $id);
		$control_numero_factura->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing cliente_id - begin
	$condition = 1;

	if($condition)
	{
		$control_cliente_id = $pageObject->getControl("cliente_id", $id);
		$control_cliente_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing monto - begin
	$condition = 1;

	if($condition)
	{
		$control_monto = $pageObject->getControl("monto", $id);
		$control_monto->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: factura_consulta_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: factura_consulta_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["numero_factura"] = $evalues["numero_factura"];
	$data["consulta_id"] = $evalues["consulta_id"];
	$data["cliente_id"] = $evalues["cliente_id"];
	$data["fecha"] = $evalues["fecha"];
	$data["monto"] = $evalues["monto"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("numero_factura_label","<label for=\"".GetInputElementId("numero_factura", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("cliente_id_label","<label for=\"".GetInputElementId("cliente_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("monto_label","<label for=\"".GetInputElementId("monto", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='factura_consulta_view.php?".$viewlink."'\"");
