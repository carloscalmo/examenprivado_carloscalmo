...
$layout->blocks["top"][] = "details";$page_layouts["cirugia_edit"] = $layout;
...
//	processing encargado - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_encargado = $pageObject->getControl("encargado", $id);
		$control_encargado->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing idtipo_cirugia - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_idtipo_cirugia = $pageObject->getControl("idtipo_cirugia", $id);
		$control_idtipo_cirugia->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: cirugia_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: cirugia_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["encargado"] = $evalues["encargado"];
	$data["ambiente_id"] = $evalues["ambiente_id"];
	$data["idtipo_cirugia"] = $evalues["idtipo_cirugia"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("encargado_label","<label for=\"".GetInputElementId("encargado", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("idtipo_cirugia_label","<label for=\"".GetInputElementId("idtipo_cirugia", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='cirugia_view.php?".$viewlink."'\"");
