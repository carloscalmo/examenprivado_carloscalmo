...
$layout->blocks["top"][] = "details";$page_layouts["municipio_departamento_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "municipio_departamento_inline_add.htm";
else
	$templatefile = "municipio_departamento_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: municipio_departamento_add.php');
...
//	processing Municipio - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_Municipio = $pageObject->getControl("Municipio", $id);
		$control_Municipio->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Municipio - end
...
//	processing Municipio - start
			...
			if($inlineAddOption)
			{
				$control_Municipio->afterSuccessfulSave();
			}
//	processing Municipio - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'municipio_departamento_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'municipio_departamento_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: municipio_departamento_".$pageObject->getPageType().".php");
...
//	Municipio
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("Municipio", $data, $keylink);
		$showValues["Municipio"] = $value;
		$showFields[] = "Municipio";
	}	
		$showRawValues["departamento"] = substr($data["departamento"],0,100);
		$showRawValues["Municipio"] = substr($data["Municipio"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "municipio_departamento";
		$options['firstTime'] = 1;
