...
$layout->blocks["top"][] = "details";$page_layouts["detalle_servicio_edit"] = $layout;
...
$templatefile = "detalle_servicio_edit.htm";
...
$keys["servicio_id"] = urldecode(postvalue("editid2"));
$savedKeys["servicio_id"] = urldecode(postvalue("editid2"));
...
//	processing servicio_id - begin
	...
	if($condition)
	{
		...
		$control_servicio_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
			...
			//	update key value
		if($control_servicio_id->getWebValue()!==false)
			$keys["servicio_id"] = $control_servicio_id->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["servicio_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: detalle_servicio_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: detalle_servicio_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("servicio_id", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='detalle_servicio_view.php?".$viewlink."'\"");
