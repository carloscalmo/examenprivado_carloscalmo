...
$layout->blocks["top"][] = "details";$page_layouts["abonos_proveedores_edit"] = $layout;
...
$templatefile = "abonos_proveedores_edit.htm";
...
$keys["factura"] = urldecode(postvalue("editid2"));
$savedKeys["factura"] = urldecode(postvalue("editid2"));
...
//	processing factura - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_factura = $pageObject->getControl("factura", $id);
		$control_factura->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_factura->getWebValue()!==false)
			$keys["factura"] = $control_factura->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["factura"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: abonos_proveedores_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: abonos_proveedores_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("factura_label","<label for=\"".GetInputElementId("factura", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("id", $data)));
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("factura", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='abonos_proveedores_view.php?".$viewlink."'\"");
