...
$layout->blocks["top"][] = "search";$page_layouts["Copy_of_lotes_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "Copy_of_lotes_search.htm";
...
$params['shortTableName'] = 'Copy_of_lotes';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("precio"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("precio")] = GetTableURL($pageObject->pSet->getLookupTable("precio"));
	
	$pageObject->fillFieldToolTips("precio");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("precio");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("precio_label","<label for=\"".GetInputElementId("precio", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("precio_label", true);
	
	$xt->assign("precio_fieldblock", true);
	$xt->assignbyref("precio_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("precio_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("precio_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_precio", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("precio");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"precio", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"precio", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("costo"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("costo")] = GetTableURL($pageObject->pSet->getLookupTable("costo"));
	
	$pageObject->fillFieldToolTips("costo");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("costo");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("costo_label","<label for=\"".GetInputElementId("costo", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("costo_label", true);
	
	$xt->assign("costo_fieldblock", true);
	$xt->assignbyref("costo_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("costo_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("costo_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_costo", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("costo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"costo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"costo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("codigo_insumo"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("codigo_insumo")] = GetTableURL($pageObject->pSet->getLookupTable("codigo_insumo"));
	
	$pageObject->fillFieldToolTips("codigo_insumo");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("codigo_insumo");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "codigo_insumo";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("codigo_insumo_label","<label for=\"".GetInputElementId("codigo_insumo", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("codigo_insumo_label", true);
	
	$xt->assign("codigo_insumo_fieldblock", true);
	$xt->assignbyref("codigo_insumo_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("codigo_insumo_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("codigo_insumo_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_codigo_insumo", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("codigo_insumo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"codigo_insumo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"codigo_insumo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
