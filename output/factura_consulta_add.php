...
$layout->blocks["top"][] = "details";$page_layouts["factura_consulta_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "factura_consulta_inline_add.htm";
else
	$templatefile = "factura_consulta_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: factura_consulta_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing numero_factura - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_numero_factura = $pageObject->getControl("numero_factura", $id);
		$control_numero_factura->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing numero_factura - end
...
//	processing cliente_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_cliente_id = $pageObject->getControl("cliente_id", $id);
		$control_cliente_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing cliente_id - end
...
//	processing monto - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_monto = $pageObject->getControl("monto", $id);
		$control_monto->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing monto - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="persona")
	{
		...
		if($avalues["cliente_id"]==""){
			$avalues["cliente_id"] = prepare_for_db("cliente_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing numero_factura - start
			...
			if($inlineAddOption)
			{
				$control_numero_factura->afterSuccessfulSave();
			}
//	processing numero_factura - end
//	processing consulta_id - start
			$inlineAddOption = true;
...
//	processing cliente_id - start
			...
			if($inlineAddOption)
			{
				$control_cliente_id->afterSuccessfulSave();
			}
//	processing cliente_id - end
//	processing fecha - start
			$inlineAddOption = true;
...
//	processing monto - start
			...
			if($inlineAddOption)
			{
				$control_monto->afterSuccessfulSave();
			}
//	processing monto - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'factura_consulta_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'factura_consulta_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: factura_consulta_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="persona")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["cliente_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	$defvalues["numero_factura"]=@$avalues["numero_factura"];
	$defvalues["consulta_id"]=@$avalues["consulta_id"];
	$defvalues["cliente_id"]=@$avalues["cliente_id"];
	$defvalues["fecha"]=@$avalues["fecha"];
	$defvalues["monto"]=@$avalues["monto"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("numero_factura"))
		$xt->assign("numero_factura_fieldblock",true);
	else
		$xt->assign("numero_factura_tabfieldblock",true);
	$xt->assign("numero_factura_label",true);
	if(isEnableSection508())
		$xt->assign_section("numero_factura_label","<label for=\"".GetInputElementId("numero_factura", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("cliente_id"))
		$xt->assign("cliente_id_fieldblock",true);
	else
		$xt->assign("cliente_id_tabfieldblock",true);
	$xt->assign("cliente_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("cliente_id_label","<label for=\"".GetInputElementId("cliente_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("monto"))
		$xt->assign("monto_fieldblock",true);
	else
		$xt->assign("monto_tabfieldblock",true);
	$xt->assign("monto_label",true);
	if(isEnableSection508())
		$xt->assign_section("monto_label","<label for=\"".GetInputElementId("monto", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["abonos_clientes"]["masterkey1"] = $data["id"];	
...
//	numero_factura
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("numero_factura", $data, $keylink);
		$showValues["numero_factura"] = $value;
		$showFields[] = "numero_factura";
	}	
//	consulta_id
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	cliente_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("cliente_id", $data, $keylink);
		$showValues["cliente_id"] = $value;
		$showFields[] = "cliente_id";
	}	
//	fecha
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	monto
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("monto", $data, $keylink);
		$showValues["monto"] = $value;
		$showFields[] = "monto";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["numero_factura"] = substr($data["numero_factura"],0,100);
		$showRawValues["consulta_id"] = substr($data["consulta_id"],0,100);
		$showRawValues["cliente_id"] = substr($data["cliente_id"],0,100);
		$showRawValues["fecha"] = substr($data["fecha"],0,100);
		$showRawValues["monto"] = substr($data["monto"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "factura_consulta";
		$options['firstTime'] = 1;
