...
add_nocache_headers();
include("include/puesto_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["puesto_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "puesto_inline_add.htm";
else
	$templatefile = "puesto_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: puesto_add.php');
...
//	processing salario - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_salario = $pageObject->getControl("salario", $id);
		$control_salario->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing salario - end
...
//	processing salario - start
			...
			if($inlineAddOption)
			{
				$control_salario->afterSuccessfulSave();
			}
//	processing salario - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'puesto_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'puesto_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: puesto_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["salario"]=@$avalues["salario"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	$xt->assign("salario_label",true);
	if(isEnableSection508())
		$xt->assign_section("salario_label","<label for=\"".GetInputElementId("salario", $id, PAGE_ADD)."\">","</label>");
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="empleado" && postvalue("field")=="puesto_id") 
	{
		$lookupFieldName = "puesto_id";
		$lookupTable = "puesto";
		$linkFieldName = "id";
		$dispfield = "nombre";
		...
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "empleado", "puesto_id");
