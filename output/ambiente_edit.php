...
$layout->blocks["top"][] = "details";$page_layouts["ambiente_edit"] = $layout;
...
$templatefile = "ambiente_edit.htm";
...
//	processing capacidad - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_capacidad = $pageObject->getControl("capacidad", $id);
		$control_capacidad->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: ambiente_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: ambiente_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["capacidad"] = $evalues["capacidad"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("capacidad_label","<label for=\"".GetInputElementId("capacidad", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='ambiente_view.php?".$viewlink."'\"");
