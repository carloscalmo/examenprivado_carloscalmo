...
$layout->blocks["top"][] = "details";$page_layouts["hospitalizacion_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "hospitalizacion_inline_add.htm";
else
	$templatefile = "hospitalizacion_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: hospitalizacion_add.php');
...
//	processing numero_cama - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_numero_cama = $pageObject->getControl("numero_cama", $id);
		$control_numero_cama->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing numero_cama - end
//	processing fecha_ingreso - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_fecha_ingreso = $pageObject->getControl("fecha_ingreso", $id);
		$control_fecha_ingreso->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing fecha_ingreso - end
//	processing fecha_egreso - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_fecha_egreso = $pageObject->getControl("fecha_egreso", $id);
		$control_fecha_egreso->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing fecha_egreso - end
...
//	processing numero_cama - start
			...
			if($inlineAddOption)
			{
				$control_numero_cama->afterSuccessfulSave();
			}
//	processing numero_cama - end
//	processing fecha_ingreso - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_fecha_ingreso->afterSuccessfulSave();
			}
//	processing fecha_ingreso - end
//	processing fecha_egreso - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_fecha_egreso->afterSuccessfulSave();
			}
//	processing fecha_egreso - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'hospitalizacion_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'hospitalizacion_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: hospitalizacion_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["numero_cama"]=@$avalues["numero_cama"];
	$defvalues["ambiente_id"]=@$avalues["ambiente_id"];
	$defvalues["consulta_id"]=@$avalues["consulta_id"];
	$defvalues["fecha_ingreso"]=@$avalues["fecha_ingreso"];
	$defvalues["fecha_egreso"]=@$avalues["fecha_egreso"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("numero_cama"))
		$xt->assign("numero_cama_fieldblock",true);
	else
		$xt->assign("numero_cama_tabfieldblock",true);
	$xt->assign("numero_cama_label",true);
	if(isEnableSection508())
		$xt->assign_section("numero_cama_label","<label for=\"".GetInputElementId("numero_cama", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("fecha_ingreso"))
		$xt->assign("fecha_ingreso_fieldblock",true);
	else
		$xt->assign("fecha_ingreso_tabfieldblock",true);
	$xt->assign("fecha_ingreso_label",true);
	if(isEnableSection508())
		$xt->assign_section("fecha_ingreso_label","<label for=\"".GetInputElementId("fecha_ingreso", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("fecha_egreso"))
		$xt->assign("fecha_egreso_fieldblock",true);
	else
		$xt->assign("fecha_egreso_tabfieldblock",true);
	$xt->assign("fecha_egreso_label",true);
	if(isEnableSection508())
		$xt->assign_section("fecha_egreso_label","<label for=\"".GetInputElementId("fecha_egreso", $id, PAGE_ADD)."\">","</label>");
...
//	numero_cama
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("numero_cama", $data, $keylink);
		$showValues["numero_cama"] = $value;
		$showFields[] = "numero_cama";
	}	
//	ambiente_id
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	fecha_ingreso
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("fecha_ingreso", $data, $keylink);
		$showValues["fecha_ingreso"] = $value;
		$showFields[] = "fecha_ingreso";
	}	
//	fecha_egreso
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("fecha_egreso", $data, $keylink);
		$showValues["fecha_egreso"] = $value;
		$showFields[] = "fecha_egreso";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["numero_cama"] = substr($data["numero_cama"],0,100);
		$showRawValues["ambiente_id"] = substr($data["ambiente_id"],0,100);
		$showRawValues["consulta_id"] = substr($data["consulta_id"],0,100);
		$showRawValues["fecha_ingreso"] = substr($data["fecha_ingreso"],0,100);
		$showRawValues["fecha_egreso"] = substr($data["fecha_egreso"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "hospitalizacion";
		$options['firstTime'] = 1;
