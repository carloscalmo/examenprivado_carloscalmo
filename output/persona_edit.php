...
$layout->blocks["top"][] = "details";$page_layouts["persona_edit"] = $layout;
...
$keys["cui"] = urldecode(postvalue("editid2"));
$savedKeys["cui"] = urldecode(postvalue("editid2"));
...
//	processing apellido - begin
	...
	if($condition)
	{
		...
		$control_apellido->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing apellido - end
//	processing fecha_nacimiento - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_fecha_nacimiento = $pageObject->getControl("fecha_nacimiento", $id);
		$control_fecha_nacimiento->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing fecha_nacimiento - end
//	processing cui - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_cui = $pageObject->getControl("cui", $id);
		$control_cui->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_cui->getWebValue()!==false)
			$keys["cui"] = $control_cui->getWebValue();
	}
//	processing cui - end
//	processing direccion - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_direccion = $pageObject->getControl("direccion", $id);
		$control_direccion->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing direccion - end
//	processing telefono - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_telefono = $pageObject->getControl("telefono", $id);
		$control_telefono->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing telefono - end
//	processing nit - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_nit = $pageObject->getControl("nit", $id);
		$control_nit->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing nit - end
//	processing genero_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_genero_id = $pageObject->getControl("genero_id", $id);
		$control_genero_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing genero_id - end
//	processing municipio_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_municipio_id = $pageObject->getControl("municipio_id", $id);
		$control_municipio_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["cui"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: persona_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["fecha_nacimiento"] = $evalues["fecha_nacimiento"];
	$data["cui"] = $evalues["cui"];
	$data["direccion"] = $evalues["direccion"];
	$data["telefono"] = $evalues["telefono"];
	$data["nit"] = $evalues["nit"];
	$data["genero_id"] = $evalues["genero_id"];
	$data["municipio_id"] = $evalues["municipio_id"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("apellido_label","<label for=\"".GetInputElementId("apellido", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_nacimiento_label","<label for=\"".GetInputElementId("fecha_nacimiento", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("cui_label","<label for=\"".GetInputElementId("cui", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("direccion_label","<label for=\"".GetInputElementId("direccion", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("telefono_label","<label for=\"".GetInputElementId("telefono", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nit_label","<label for=\"".GetInputElementId("nit", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("genero_id_label","<label for=\"".GetInputElementId("genero_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("municipio_id_label","<label for=\"".GetInputElementId("municipio_id", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='persona_view.php?".$viewlink."'\"");
