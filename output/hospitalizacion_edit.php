...
$layout->blocks["top"][] = "details";$page_layouts["hospitalizacion_edit"] = $layout;
...
$templatefile = "hospitalizacion_edit.htm";
...
$keys["consulta_id"] = urldecode(postvalue("editid2"));
$savedKeys["consulta_id"] = urldecode(postvalue("editid2"));
...
//	processing numero_cama - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_numero_cama = $pageObject->getControl("numero_cama", $id);
		$control_numero_cama->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing numero_cama - end
//	processing ambiente_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_ambiente_id = $pageObject->getControl("ambiente_id", $id);
		$control_ambiente_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing ambiente_id - end
//	processing consulta_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_consulta_id = $pageObject->getControl("consulta_id", $id);
		$control_consulta_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			//	update key value
		if($control_consulta_id->getWebValue()!==false)
			$keys["consulta_id"] = $control_consulta_id->getWebValue();
	}
//	processing consulta_id - end
//	processing fecha_ingreso - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_fecha_ingreso = $pageObject->getControl("fecha_ingreso", $id);
		$control_fecha_ingreso->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

			}
//	processing fecha_ingreso - end
//	processing fecha_egreso - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_fecha_egreso = $pageObject->getControl("fecha_egreso", $id);
		$control_fecha_egreso->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["consulta_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: hospitalizacion_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: hospitalizacion_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["numero_cama"] = $evalues["numero_cama"];
	$data["ambiente_id"] = $evalues["ambiente_id"];
	$data["consulta_id"] = $evalues["consulta_id"];
	$data["fecha_ingreso"] = $evalues["fecha_ingreso"];
	$data["fecha_egreso"] = $evalues["fecha_egreso"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("numero_cama_label","<label for=\"".GetInputElementId("numero_cama", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ambiente_id_label","<label for=\"".GetInputElementId("ambiente_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("consulta_id_label","<label for=\"".GetInputElementId("consulta_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_ingreso_label","<label for=\"".GetInputElementId("fecha_ingreso", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_egreso_label","<label for=\"".GetInputElementId("fecha_egreso", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("id", $data)));
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("consulta_id", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='hospitalizacion_view.php?".$viewlink."'\"");
