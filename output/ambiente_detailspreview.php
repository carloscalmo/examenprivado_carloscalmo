...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["ambiente_detailspreview"] = $layout;
...
if($mastertable == "tipo_ambiente")
{
	$where = "";
		$where .= GetFullFieldName("idtipo_ambiente", $strTableName, false)."=".make_db_value("idtipo_ambiente",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	capacidad - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("capacidad", $data, $keylink);
			$row["capacidad_value"] = $value;
	//	idtipo_ambiente - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idtipo_ambiente", $data, $keylink);
			$row["idtipo_ambiente_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("ambiente_detailspreview.htm");
