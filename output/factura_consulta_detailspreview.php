...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["factura_consulta_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("cliente_id", $strTableName, false)."=".make_db_value("cliente_id",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	cliente_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("cliente_id", $data, $keylink);
			$row["cliente_id_value"] = $value;
	//	fecha - Short Date
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template("factura_consulta_detailspreview.htm");
