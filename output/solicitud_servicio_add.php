...
$layout->blocks["top"][] = "details";$page_layouts["solicitud_servicio_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "solicitud_servicio_inline_add.htm";
else
	$templatefile = "solicitud_servicio_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: solicitud_servicio_add.php');
...
//	processing servicio_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_servicio_id = $pageObject->getControl("servicio_id", $id);
		$control_servicio_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing servicio_id - end
//	processing solicitante - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_solicitante = $pageObject->getControl("solicitante", $id);
		$control_solicitante->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing solicitante - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
	{
		...
		if($avalues["solicitante"]==""){
			$avalues["solicitante"] = prepare_for_db("solicitante",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="servicio")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["servicio_id"]==""){
			$avalues["servicio_id"] = prepare_for_db("servicio_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing servicio_id - start
			...
			if($inlineAddOption)
			{
				$control_servicio_id->afterSuccessfulSave();
			}
//	processing servicio_id - end
//	processing solicitante - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_solicitante->afterSuccessfulSave();
			}
//	processing solicitante - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'solicitud_servicio_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'solicitud_servicio_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: solicitud_servicio_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["solicitante"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="servicio")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["servicio_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["servicio_id"]=@$avalues["servicio_id"];
	$defvalues["solicitante"]=@$avalues["solicitante"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("servicio_id"))
		$xt->assign("servicio_id_fieldblock",true);
	else
		$xt->assign("servicio_id_tabfieldblock",true);
	$xt->assign("servicio_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("servicio_id_label","<label for=\"".GetInputElementId("servicio_id", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("solicitante"))
		$xt->assign("solicitante_fieldblock",true);
	else
		$xt->assign("solicitante_tabfieldblock",true);
	$xt->assign("solicitante_label",true);
	if(isEnableSection508())
		$xt->assign_section("solicitante_label","<label for=\"".GetInputElementId("solicitante", $id, PAGE_ADD)."\">","</label>");
...
//	servicio_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("servicio_id", $data, $keylink);
		$showValues["servicio_id"] = $value;
		$showFields[] = "servicio_id";
	}	
//	solicitante
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("solicitante", $data, $keylink);
		$showValues["solicitante"] = $value;
		...
		$showRawValues["servicio_id"] = substr($data["servicio_id"],0,100);
		$showRawValues["solicitante"] = substr($data["solicitante"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "solicitud_servicio";
		$options['firstTime'] = 1;
