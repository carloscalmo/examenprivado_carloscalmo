...
add_nocache_headers();
include("include/diagnostico_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["diagnostico_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "diagnostico_inline_add.htm";
else
	$templatefile = "diagnostico_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: diagnostico_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'diagnostico_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'diagnostico_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: diagnostico_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "diagnostico";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="consulta" && postvalue("field")=="diagnostico_id") 
	{
		$lookupFieldName = "diagnostico_id";
		$lookupTable = "diagnostico";
		$linkFieldName = "id";
		$dispfield = "nombre";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("consulta", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "consulta", "diagnostico_id");
