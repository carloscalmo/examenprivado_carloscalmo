...
$layout->blocks["top"][] = "export";$page_layouts["Copy_of_consulta_export"] = $layout;
...
$xt->display("Copy_of_consulta_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=Copy_of_consulta.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_consulta.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_consulta.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["tipo_consulta_id"] = $pageObject->showDBValue("tipo_consulta_id", $row);
			$values["id1"] = $pageObject->showDBValue("id1", $row);
			$values["nombre"] = $pageObject->showDBValue("nombre", $row);
			$values["apellido"] = $pageObject->showDBValue("apellido", $row);
			$values["cui"] = $pageObject->showDBValue("cui", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=Copy_of_consulta.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["tipo_consulta_id"] = $pageObject->getViewControl("tipo_consulta_id")->showDBValue($row, "");
			$values["id1"] = $pageObject->getViewControl("id1")->showDBValue($row, "");
			$values["nombre"] = $pageObject->getViewControl("nombre")->showDBValue($row, "");
			$values["apellido"] = $pageObject->getViewControl("apellido")->showDBValue($row, "");
			$values["cui"] = $pageObject->getViewControl("cui")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["tipo_consulta_id"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Cui").'</td>';	
	}
	else
	{
		echo "<td>"."Código"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["cui"]);
					else
						echo $values["cui"];
			echo '</td>';
			echo "</tr>";
