...
$layout->blocks["top"][] = "details";$page_layouts["lotes_edit"] = $layout;
...
//	processing produccion - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_produccion = $pageObject->getControl("produccion", $id);
		$control_produccion->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing produccion - end
//	processing caducidad - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_caducidad = $pageObject->getControl("caducidad", $id);
		$control_caducidad->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing caducidad - end
//	processing expirado - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_expirado = $pageObject->getControl("expirado", $id);
		$control_expirado->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing expirado - end
//	processing proveedor - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_proveedor = $pageObject->getControl("proveedor", $id);
		$control_proveedor->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing proveedor - end
//	processing insumo - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_insumo = $pageObject->getControl("insumo", $id);
		$control_insumo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing insumo - end
//	processing cantidad - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_cantidad = $pageObject->getControl("cantidad", $id);
		$control_cantidad->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing cantidad - end
//	processing precio - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_precio = $pageObject->getControl("precio", $id);
		$control_precio->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing precio - end
//	processing costo - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_costo = $pageObject->getControl("costo", $id);
		$control_costo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: lotes_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: lotes_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["produccion"] = $evalues["produccion"];
	$data["caducidad"] = $evalues["caducidad"];
	$data["expirado"] = $evalues["expirado"];
	$data["proveedor"] = $evalues["proveedor"];
	$data["insumo"] = $evalues["insumo"];
	$data["cantidad"] = $evalues["cantidad"];
	$data["precio"] = $evalues["precio"];
	$data["costo"] = $evalues["costo"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("codigo_label","<label for=\"".GetInputElementId("codigo", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("produccion_label","<label for=\"".GetInputElementId("produccion", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("caducidad_label","<label for=\"".GetInputElementId("caducidad", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("expirado_label","<label for=\"".GetInputElementId("expirado", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("proveedor_label","<label for=\"".GetInputElementId("proveedor", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("insumo_label","<label for=\"".GetInputElementId("insumo", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("cantidad_label","<label for=\"".GetInputElementId("cantidad", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("precio_label","<label for=\"".GetInputElementId("precio", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("costo_label","<label for=\"".GetInputElementId("costo", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='lotes_view.php?".$viewlink."'\"");
