...
$layout->blocks["top"][] = "details";$page_layouts["Copy_of_departamento_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Copy_of_departamento_inline_add.htm";
else
	$templatefile = "Copy_of_departamento_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Copy_of_departamento_add.php');
...
//	processing nombre1 - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_nombre1 = $pageObject->getControl("nombre1", $id);
		$control_nombre1->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing nombre1 - end
//	processing departamento - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_departamento = $pageObject->getControl("departamento", $id);
		$control_departamento->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing departamento - end
...
//	processing nombre1 - start
			...
			if($inlineAddOption)
			{
				$control_nombre1->afterSuccessfulSave();
			}
//	processing nombre1 - end
//	processing departamento - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_departamento->afterSuccessfulSave();
			}
//	processing departamento - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Copy_of_departamento_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Copy_of_departamento_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Copy_of_departamento_".$pageObject->getPageType().".php");
...
//	nombre1
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("nombre1", $data, $keylink);
		$showValues["nombre1"] = $value;
		$showFields[] = "nombre1";
	}	
//	departamento
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("departamento", $data, $keylink);
		$showValues["departamento"] = $value;
		$showFields[] = "departamento";
	}	
		...
		$showRawValues["nombre1"] = substr($data["nombre1"],0,100);
		$showRawValues["departamento"] = substr($data["departamento"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Copy of departamento";
		$options['firstTime'] = 1;
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="proveedores" && postvalue("field")=="municipio_id") 
	{
		$lookupFieldName = "municipio_id";
		$lookupTable = "Copy of departamento";
		$linkFieldName = "id1";
		$dispfield = "concat(municipio.nombre,' ',departamento.nombre)";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("proveedores", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "proveedores", "municipio_id");
			...
			$LookupSQL = "select ";
			$LookupSQL .= GetFullFieldName("id1", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",concat(municipio.nombre,' ',departamento.nombre)";
		}
	}
	if(postvalue("table")=="persona" && postvalue("field")=="municipio_id") 
	{
		$lookupFieldName = "municipio_id";
		$lookupTable = "Copy of departamento";
		$linkFieldName = "id1";
		$dispfield = " concat(municipio.nombre,' ',departamento.nombre) ";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("persona", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "persona", "municipio_id");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
