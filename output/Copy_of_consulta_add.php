...
$layout->blocks["top"][] = "details";$page_layouts["Copy_of_consulta_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Copy_of_consulta_inline_add.htm";
else
	$templatefile = "Copy_of_consulta_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: Copy_of_consulta_add.php');
...
//	processing id1 - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_id1 = $pageObject->getControl("id1", $id);
		$control_id1->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing id1 - end
...
//	processing id1 - start
			...
			if($inlineAddOption)
			{
				$control_id1->afterSuccessfulSave();
			}
//	processing id1 - end
//	processing nombre - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'Copy_of_consulta_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'Copy_of_consulta_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Copy_of_consulta_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Copy of consulta";
		$options['firstTime'] = 1;
