...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["abonos_proveedores_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("factura", $strTableName, false)."=".make_db_value("factura",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	factura - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("factura", $data, $keylink);
			$row["factura_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("abonos_proveedores_detailspreview.htm");
