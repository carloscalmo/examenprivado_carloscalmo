...
$layout->blocks["top"][] = "details";$page_layouts["consulta_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "consulta_inline_add.htm";
else
	$templatefile = "consulta_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: consulta_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing tipo_consulta_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_tipo_consulta_id = $pageObject->getControl("tipo_consulta_id", $id);
		$control_tipo_consulta_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tipo_consulta_id - end
//	processing paciente - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_paciente = $pageObject->getControl("paciente", $id);
		$control_paciente->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing paciente - end
...
//	processing medico_encargado - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_medico_encargado = $pageObject->getControl("medico_encargado", $id);
		$control_medico_encargado->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing medico_encargado - end
//	processing diagnostico_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_diagnostico_id = $pageObject->getControl("diagnostico_id", $id);
		$control_diagnostico_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing diagnostico_id - end
//	processing estado_retirada - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_estado_retirada = $pageObject->getControl("estado_retirada", $id);
		$control_estado_retirada->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing estado_retirada - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="diagnostico")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["diagnostico_id"]==""){
			$avalues["diagnostico_id"] = prepare_for_db("diagnostico_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
	{
		...
		if($avalues["medico_encargado"]==""){
			$avalues["medico_encargado"] = prepare_for_db("medico_encargado",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="persona")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["paciente"]==""){
			$avalues["paciente"] = prepare_for_db("paciente",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="tipo_consulta")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["tipo_consulta_id"]==""){
			$avalues["tipo_consulta_id"] = prepare_for_db("tipo_consulta_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing tipo_consulta_id - start
			...
			if($inlineAddOption)
			{
				$control_tipo_consulta_id->afterSuccessfulSave();
			}
//	processing tipo_consulta_id - end
//	processing paciente - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_paciente->afterSuccessfulSave();
			}
//	processing paciente - end
//	processing fecha - start
			$inlineAddOption = true;
...
//	processing medico_encargado - start
			...
			if($inlineAddOption)
			{
				$control_medico_encargado->afterSuccessfulSave();
			}
//	processing medico_encargado - end
//	processing diagnostico_id - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_diagnostico_id->afterSuccessfulSave();
			}
//	processing diagnostico_id - end
//	processing estado_retirada - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_estado_retirada->afterSuccessfulSave();
			}
//	processing estado_retirada - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'consulta_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'consulta_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: consulta_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["paciente"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["paciente"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="diagnostico")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["diagnostico_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="empleado")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["medico_encargado"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="persona")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["paciente"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="tipo_consulta")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["tipo_consulta_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["paciente"]=@$avalues["paciente"];
	$defvalues["estado_retirada"]=@$avalues["estado_retirada"];
	$defvalues["medico_encargado"]=@$avalues["medico_encargado"];
	$defvalues["diagnostico_id"]=@$avalues["diagnostico_id"];
	$defvalues["tipo_consulta_id"]=@$avalues["tipo_consulta_id"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("paciente_label","<label for=\"".GetInputElementId("paciente", $id, PAGE_ADD)."\">","</label>");
	...
	$xt->assign("estado_retirada_label",true);
	if(isEnableSection508())
		$xt->assign_section("estado_retirada_label","<label for=\"".GetInputElementId("estado_retirada", $id, PAGE_ADD)."\">","</label>");
	...
	$xt->assign("medico_encargado_label",true);
	if(isEnableSection508())
		$xt->assign_section("medico_encargado_label","<label for=\"".GetInputElementId("medico_encargado", $id, PAGE_ADD)."\">","</label>");
	...
	$xt->assign("diagnostico_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("diagnostico_id_label","<label for=\"".GetInputElementId("diagnostico_id", $id, PAGE_ADD)."\">","</label>");
	...
	$xt->assign("tipo_consulta_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("tipo_consulta_id_label","<label for=\"".GetInputElementId("tipo_consulta_id", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["cirugia"]["masterkey1"] = $data["id"];	
	$showDetailKeys["hospitalizacion"]["masterkey1"] = $data["id"];	
	$showDetailKeys["solicitud_servicio"]["masterkey1"] = $data["id"];	
...
//	estado_retirada
	...
	{	
		...
		$showValues["estado_retirada"] = $value;
		$showFields[] = "estado_retirada";
	}	
//	medico_encargado
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showValues["medico_encargado"] = $value;
		$showFields[] = "medico_encargado";
	}	
//	diagnostico_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showValues["diagnostico_id"] = $value;
		$showFields[] = "diagnostico_id";
	}	
//	tipo_consulta_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showValues["tipo_consulta_id"] = $value;
		$showFields[] = "tipo_consulta_id";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["fecha"] = substr($data["fecha"],0,100);
		$showRawValues["paciente"] = substr($data["paciente"],0,100);
		$showRawValues["estado_retirada"] = substr($data["estado_retirada"],0,100);
		$showRawValues["medico_encargado"] = substr($data["medico_encargado"],0,100);
		$showRawValues["diagnostico_id"] = substr($data["diagnostico_id"],0,100);
		$showRawValues["tipo_consulta_id"] = substr($data["tipo_consulta_id"],0,100);
