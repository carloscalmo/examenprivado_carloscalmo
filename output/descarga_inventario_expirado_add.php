...
add_nocache_headers();
include("include/descarga_inventario_expirado_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["descarga_inventario_expirado_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "descarga_inventario_expirado_inline_add.htm";
else
	$templatefile = "descarga_inventario_expirado_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: descarga_inventario_expirado_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing lotes - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_lotes = $pageObject->getControl("lotes", $id);
		$control_lotes->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing lotes - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="usuarios")
	{
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="lotes")
	{
		...
		if($avalues["lotes"]==""){
			$avalues["lotes"] = prepare_for_db("lotes",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing lotes - start
			...
			if($inlineAddOption)
			{
				$control_lotes->afterSuccessfulSave();
			}
//	processing lotes - end
//	processing fecha - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'descarga_inventario_expirado_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'descarga_inventario_expirado_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: descarga_inventario_expirado_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="lotes")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["lotes"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("lotes"))
		$xt->assign("lotes_fieldblock",true);
	else
		$xt->assign("lotes_tabfieldblock",true);
	$xt->assign("lotes_label",true);
	if(isEnableSection508())
		$xt->assign_section("lotes_label","<label for=\"".GetInputElementId("lotes", $id, PAGE_ADD)."\">","</label>");
...
//	lotes
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("lotes", $data, $keylink);
		$showValues["lotes"] = $value;
		$showFields[] = "lotes";
	}	
//	responsable
	$display = false;
	...
	{	
		...
		$showRawValues["lotes"] = substr($data["lotes"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "descarga_inventario_expirado";
		$options['firstTime'] = 1;
