...
$layout->blocks["top"][] = "details";$page_layouts["persona_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "persona_inline_add.htm";
else
	$templatefile = "persona_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: persona_add.php');
...
//	processing nombre - start
	...
	if($inlineAddOption)
	{
		...
		$control_nombre->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing apellido - start
	...
	if($inlineAddOption)
	{
		$control_apellido = $pageObject->getControl("apellido", $id);
		$control_apellido->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing apellido - end
//	processing cui - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_cui = $pageObject->getControl("cui", $id);
		$control_cui->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing cui - end
//	processing direccion - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_direccion = $pageObject->getControl("direccion", $id);
		$control_direccion->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing direccion - end
//	processing municipio_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_municipio_id = $pageObject->getControl("municipio_id", $id);
		$control_municipio_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing municipio_id - end
//	processing telefono - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_telefono = $pageObject->getControl("telefono", $id);
		$control_telefono->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing telefono - end
//	processing nit - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_nit = $pageObject->getControl("nit", $id);
		$control_nit->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing nit - end
//	processing fecha_nacimiento - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_fecha_nacimiento = $pageObject->getControl("fecha_nacimiento", $id);
		$control_fecha_nacimiento->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing fecha_nacimiento - end
//	processing genero_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_genero_id = $pageObject->getControl("genero_id", $id);
		$control_genero_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing genero_id - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="municipio")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["municipio_id"]==""){
			$avalues["municipio_id"] = prepare_for_db("municipio_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="genero")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["genero_id"]==""){
			$avalues["genero_id"] = prepare_for_db("genero_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing apellido - start
			...
			if($inlineAddOption)
			{
				$control_apellido->afterSuccessfulSave();
			}
//	processing apellido - end
//	processing cui - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_cui->afterSuccessfulSave();
			}
//	processing cui - end
//	processing direccion - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_direccion->afterSuccessfulSave();
			}
//	processing direccion - end
//	processing municipio_id - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_municipio_id->afterSuccessfulSave();
			}
//	processing municipio_id - end
//	processing telefono - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_telefono->afterSuccessfulSave();
			}
//	processing telefono - end
//	processing nit - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_nit->afterSuccessfulSave();
			}
//	processing nit - end
//	processing fecha_nacimiento - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_fecha_nacimiento->afterSuccessfulSave();
			}
//	processing fecha_nacimiento - end
//	processing genero_id - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_genero_id->afterSuccessfulSave();
			}
//	processing genero_id - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'persona_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'persona_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: persona_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["cui"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["cui"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="municipio")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["municipio_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="genero")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["genero_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["fecha_nacimiento"]=@$avalues["fecha_nacimiento"];
	$defvalues["cui"]=@$avalues["cui"];
	$defvalues["direccion"]=@$avalues["direccion"];
	$defvalues["telefono"]=@$avalues["telefono"];
	$defvalues["nit"]=@$avalues["nit"];
	$defvalues["genero_id"]=@$avalues["genero_id"];
	$defvalues["municipio_id"]=@$avalues["municipio_id"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("nombre_label","<label for=\"".GetInputElementId("nombre", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("apellido_label","<label for=\"".GetInputElementId("apellido", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_nacimiento_label","<label for=\"".GetInputElementId("fecha_nacimiento", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("cui_label","<label for=\"".GetInputElementId("cui", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("direccion_label","<label for=\"".GetInputElementId("direccion", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("telefono_label","<label for=\"".GetInputElementId("telefono", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nit_label","<label for=\"".GetInputElementId("nit", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("genero_id_label","<label for=\"".GetInputElementId("genero_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("municipio_id_label","<label for=\"".GetInputElementId("municipio_id", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["consulta"]["masterkey1"] = $data["id"];	
	$showDetailKeys["empleado"]["masterkey1"] = $data["id"];	
	$showDetailKeys["factura_consulta"]["masterkey1"] = $data["id"];	
...
//	fecha_nacimiento
	...
	{	
		...
		$showValues["fecha_nacimiento"] = $value;
		$showFields[] = "fecha_nacimiento";
	}	
//	cui
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	direccion
	...
	{	
		...
		$showValues["direccion"] = $value;
		$showFields[] = "direccion";
	}	
//	telefono
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	genero_id
	...
	{	
		...
		$showValues["genero_id"] = $value;
		$showFields[] = "genero_id";
	}	
//	municipio_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showValues["municipio_id"] = $value;
		$showFields[] = "municipio_id";
	}	
		...
		$showRawValues["fecha_nacimiento"] = substr($data["fecha_nacimiento"],0,100);
		$showRawValues["cui"] = substr($data["cui"],0,100);
		$showRawValues["direccion"] = substr($data["direccion"],0,100);
		$showRawValues["telefono"] = substr($data["telefono"],0,100);
		$showRawValues["nit"] = substr($data["nit"],0,100);
		$showRawValues["genero_id"] = substr($data["genero_id"],0,100);
		$showRawValues["municipio_id"] = substr($data["municipio_id"],0,100);
...
function GetAddedDataLookupQuery($pageObject, $keys, $forLookup)
{
	...
	if(postvalue("table")=="empleado" && postvalue("field")=="persona_id") 
	{
		$lookupFieldName = "persona_id";
		$lookupTable = "persona";
		$linkFieldName = "id";
		$dispfield = "concat(nombre,' ',apellido)";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("empleado", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "empleado", "persona_id");
			...
			$LookupSQL .= GetFullFieldName("id", $strTableName, true);
				...
				$LookupSQL .= ",concat(nombre,' ',apellido)";
		}
	}
	if(postvalue("table")=="factura_consulta" && postvalue("field")=="cliente_id") 
	{
		$lookupFieldName = "cliente_id";
		$lookupTable = "persona";
		$linkFieldName = "id";
		$dispfield = "concat(nit,' ', nombre, apellido)";
		// Table which contain lookup field!
		$lookupPSet = new ProjectSettings("factura_consulta", PAGE_ADD);
		$nLookupType = $lookupPSet->getLookupType($lookupFieldName);
		if($nLookupType == LT_QUERY)
		{
			if($lookupPSet->getCustomDisplay($lookupFieldName))
				$pageObject->pSet->getSQLQuery()->AddCustomExpression($dispfield, $pageObject->pSet, "factura_consulta", "cliente_id");
			$lookupQueryObj = $pageObject->pSet->getSQLQuery()->CloneObject();
			if(!$noBlobReplace)
				$lookupQueryObj->ReplaceFieldsWithDummies($pageObject->pSet->getBinaryFieldsIndices());
		}else{
			...
			$LookupSQL .= GetFullFieldName("id", $strTableName, true);
					if($linkFieldName != $dispfield)
				$LookupSQL .= ",concat(nit,' ', nombre, apellido)";
		}
	}
