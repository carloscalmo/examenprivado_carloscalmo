...
$layout->blocks["top"][] = "details";$page_layouts["factura_compra_edit"] = $layout;
...
$templatefile = "factura_compra_edit.htm";
...
$keys["numero_factura"] = urldecode(postvalue("editid2"));
$savedKeys["numero_factura"] = urldecode(postvalue("editid2"));
...
//	processing numero_factura - begin
	...
	if($condition)
	{
		...
		$control_numero_factura->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
			...
			//	update key value
		if($control_numero_factura->getWebValue()!==false)
			$keys["numero_factura"] = $control_numero_factura->getWebValue();
	}
...
//	processing total_factura - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_total_factura = $pageObject->getControl("total_factura", $id);
		$control_total_factura->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["numero_factura"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: factura_compra_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: factura_compra_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["total_factura"] = $evalues["total_factura"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("total_factura_label","<label for=\"".GetInputElementId("total_factura", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("show_key2", htmlspecialchars($pageObject->showDBValue("numero_factura", $data)));
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='factura_compra_view.php?".$viewlink."'\"");
