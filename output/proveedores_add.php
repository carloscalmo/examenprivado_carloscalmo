...
$layout->blocks["top"][] = "details";$page_layouts["proveedores_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "proveedores_inline_add.htm";
else
	$templatefile = "proveedores_add.htm";
...
//	processing saldo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_saldo = $pageObject->getControl("saldo", $id);
		$control_saldo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing saldo - end
...
//	processing saldo - start
			...
			if($inlineAddOption)
			{
				$control_saldo->afterSuccessfulSave();
			}
//	processing saldo - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'proveedores_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'proveedores_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: proveedores_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["nombre"]=postvalue("copyid2");
		$copykeys["nit"]=postvalue("copyid3");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["nombre"]=postvalue("editid2");
		$copykeys["nit"]=postvalue("editid3");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("saldo_label","<label for=\"".GetInputElementId("saldo", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["factura_compra"]["masterkey1"] = $data["id"];	
...
//	municipio_id
	...
	{	
		...
		$showRawValues["saldo"] = substr($data["saldo"],0,100);
