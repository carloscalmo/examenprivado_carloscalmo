...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["solicitud_servicio_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("solicitante", $strTableName, false)."=".make_db_value("solicitante",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	solicitante - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("solicitante", $data, $keylink);
			$row["solicitante_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("solicitud_servicio_detailspreview.htm");
