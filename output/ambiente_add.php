...
add_nocache_headers();
include("include/ambiente_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["ambiente_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "ambiente_inline_add.htm";
else
	$templatefile = "ambiente_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: ambiente_add.php');
...
//	processing capacidad - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_capacidad = $pageObject->getControl("capacidad", $id);
		$control_capacidad->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing capacidad - end
//	processing idtipo_ambiente - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_idtipo_ambiente = $pageObject->getControl("idtipo_ambiente", $id);
		$control_idtipo_ambiente->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing idtipo_ambiente - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="tipo_ambiente")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["idtipo_ambiente"]==""){
			$avalues["idtipo_ambiente"] = prepare_for_db("idtipo_ambiente",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing capacidad - start
			...
			if($inlineAddOption)
			{
				$control_capacidad->afterSuccessfulSave();
			}
//	processing capacidad - end
//	processing idtipo_ambiente - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_idtipo_ambiente->afterSuccessfulSave();
			}
//	processing idtipo_ambiente - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'ambiente_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'ambiente_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ambiente_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="tipo_ambiente")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["idtipo_ambiente"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["capacidad"]=@$avalues["capacidad"];
	$defvalues["idtipo_ambiente"]=@$avalues["idtipo_ambiente"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("capacidad"))
		$xt->assign("capacidad_fieldblock",true);
	else
		$xt->assign("capacidad_tabfieldblock",true);
	$xt->assign("capacidad_label",true);
	if(isEnableSection508())
		$xt->assign_section("capacidad_label","<label for=\"".GetInputElementId("capacidad", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("idtipo_ambiente"))
		$xt->assign("idtipo_ambiente_fieldblock",true);
	else
		$xt->assign("idtipo_ambiente_tabfieldblock",true);
	$xt->assign("idtipo_ambiente_label",true);
	if(isEnableSection508())
		$xt->assign_section("idtipo_ambiente_label","<label for=\"".GetInputElementId("idtipo_ambiente", $id, PAGE_ADD)."\">","</label>");
...
//	capacidad
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("capacidad", $data, $keylink);
		$showValues["capacidad"] = $value;
		$showFields[] = "capacidad";
	}	
//	idtipo_ambiente
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("idtipo_ambiente", $data, $keylink);
		$showValues["idtipo_ambiente"] = $value;
		$showFields[] = "idtipo_ambiente";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["nombre"] = substr($data["nombre"],0,100);
		$showRawValues["capacidad"] = substr($data["capacidad"],0,100);
		$showRawValues["idtipo_ambiente"] = substr($data["idtipo_ambiente"],0,100);
