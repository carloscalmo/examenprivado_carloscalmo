...
$layout->blocks["top"][] = "pdf";$page_layouts["empleado_print"] = $layout;
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		foreach(@$_REQUEST["mdelete"] as $ind)
		{
			...
			$keys["persona_id"]=refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys["persona_id"]=urldecode($arr[1]);
			$selected_recs[]=$keys;
		}
	}
...
$arr = array();
$arr['fName'] = "profesion";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("profesion");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "puesto_id";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("puesto_id");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "persona_id";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("persona_id");
$fieldsArr[] = $arr;
...
//	persona_id - 
			$record["persona_id_value"] = $pageObject->showDBValue("persona_id", $data, $keylink);
			$record["persona_id_class"] = $pageObject->fieldClass("persona_id");
//	profesion - 
			$record["profesion_value"] = $pageObject->showDBValue("profesion", $data, $keylink);
			$record["profesion_class"] = $pageObject->fieldClass("profesion");
//	puesto_id - 
			$record["puesto_id_value"] = $pageObject->showDBValue("puesto_id", $data, $keylink);
			$record["puesto_id_class"] = $pageObject->fieldClass("puesto_id");
			if($col<$colsonpage)
...
//	include proper masterprint.php code
	...
	$params = array("detailtable"=>"empleado","keys"=>$masterkeys);
	$master = array();
...
if($mastertable == "puesto")
{
//	include proper masterprint.php code
	include("include/puesto_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"empleado","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_puesto";
	$master["params"] = $params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$layout = new TLayout("masterprint","Rounded1BlueWave","MobileBlueWave");
...
$layout->skins["mastergrid"] = "grid";
$layout->blocks["bare"][] = "mastergrid";$page_layouts["puesto_masterprint"] = $layout;

	$layout = GetPageLayout("puesto", 'masterprint');
	if($layout)
	{
		$rtl = $pageObject->xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
		$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
$xt->assign("persona_id_fieldheadercolumn",true);
$xt->assign("persona_id_fieldheader",true);
$xt->assign("persona_id_fieldcolumn",true);
$xt->assign("persona_id_fieldfootercolumn",true);
$xt->assign("profesion_fieldheadercolumn",true);
$xt->assign("profesion_fieldheader",true);
$xt->assign("profesion_fieldcolumn",true);
$xt->assign("profesion_fieldfootercolumn",true);
$xt->assign("puesto_id_fieldheadercolumn",true);
$xt->assign("puesto_id_fieldheader",true);
$xt->assign("puesto_id_fieldcolumn",true);
$xt->assign("puesto_id_fieldfootercolumn",true);
