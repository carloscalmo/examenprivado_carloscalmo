...
$layout->blocks["top"][] = "pdf";$page_layouts["hospitalizacion_print"] = $layout;
...
//	ambiente_id - 
			$record["ambiente_id_value"] = $pageObject->showDBValue("ambiente_id", $data, $keylink);
			$record["ambiente_id_class"] = $pageObject->fieldClass("ambiente_id");
//	numero_cama - 
			$record["numero_cama_value"] = $pageObject->showDBValue("numero_cama", $data, $keylink);
			$record["numero_cama_class"] = $pageObject->fieldClass("numero_cama");
//	fecha_ingreso - Short Date
			$record["fecha_ingreso_value"] = $pageObject->showDBValue("fecha_ingreso", $data, $keylink);
			$record["fecha_ingreso_class"] = $pageObject->fieldClass("fecha_ingreso");
//	fecha_egreso - Short Date
			$record["fecha_egreso_value"] = $pageObject->showDBValue("fecha_egreso", $data, $keylink);
			$record["fecha_egreso_class"] = $pageObject->fieldClass("fecha_egreso");
			if($col<$colsonpage)
...
//	include proper masterprint.php code
	...
	$params = array("detailtable"=>"hospitalizacion","keys"=>$masterkeys);
	$master = array();
...
{
//	include proper masterprint.php code
	include("include/ambiente_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$params = array("detailtable"=>"hospitalizacion","keys"=>$masterkeys);
	$master = array();
	$master["func"] = "DisplayMasterTableInfo_ambiente";
	$master["params"] = $params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$layout = new TLayout("masterprint","Rounded1BlueWave","MobileBlueWave");
...
$layout->skins["mastergrid"] = "grid";
$layout->blocks["bare"][] = "mastergrid";$page_layouts["ambiente_masterprint"] = $layout;

	$layout = GetPageLayout("ambiente", 'masterprint');
	if($layout)
	{
		$rtl = $pageObject->xt->getReadingOrder() == 'RTL' ? 'RTL' : '';
		$xt->cssFiles[] = array("stylepath" => "styles/".$layout->style.'/style'.$rtl.".css"
...
$xt->assign("ambiente_id_fieldheadercolumn",true);
$xt->assign("ambiente_id_fieldheader",true);
$xt->assign("ambiente_id_fieldcolumn",true);
$xt->assign("ambiente_id_fieldfootercolumn",true);
$xt->assign("numero_cama_fieldheadercolumn",true);
$xt->assign("numero_cama_fieldheader",true);
$xt->assign("numero_cama_fieldcolumn",true);
$xt->assign("numero_cama_fieldfootercolumn",true);
$xt->assign("fecha_ingreso_fieldheadercolumn",true);
$xt->assign("fecha_ingreso_fieldheader",true);
$xt->assign("fecha_ingreso_fieldcolumn",true);
$xt->assign("fecha_ingreso_fieldfootercolumn",true);
$xt->assign("fecha_egreso_fieldheadercolumn",true);
$xt->assign("fecha_egreso_fieldheader",true);
$xt->assign("fecha_egreso_fieldcolumn",true);
$xt->assign("fecha_egreso_fieldfootercolumn",true);
