...
add_nocache_headers();
include("include/factura_compra_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["factura_compra_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "factura_compra_inline_add.htm";
else
	$templatefile = "factura_compra_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: factura_compra_add.php');
...
//	processing total_factura - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_total_factura = $pageObject->getControl("total_factura", $id);
		$control_total_factura->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing total_factura - end
...
//	processing total_factura - start
			...
			if($inlineAddOption)
			{
				$control_total_factura->afterSuccessfulSave();
			}
//	processing total_factura - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'factura_compra_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'factura_compra_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: factura_compra_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["numero_factura"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["numero_factura"]=postvalue("editid2");
	}
...
if($readavalues)
{
	...
	$defvalues["total_factura"]=@$avalues["total_factura"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("total_factura"))
		$xt->assign("total_factura_fieldblock",true);
	else
		$xt->assign("total_factura_tabfieldblock",true);
	$xt->assign("total_factura_label",true);
	if(isEnableSection508())
		$xt->assign_section("total_factura_label","<label for=\"".GetInputElementId("total_factura", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["abonos_proveedores"]["masterkey1"] = $data["id"];	
...
//	total_factura
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("total_factura", $data, $keylink);
		$showValues["total_factura"] = $value;
		$showFields[] = "total_factura";
	}	
//	proveedor
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		...
		$showRawValues["total_factura"] = substr($data["total_factura"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "factura_compra";
		$options['firstTime'] = 1;
