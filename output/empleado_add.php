...
add_nocache_headers();
include("include/empleado_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["empleado_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "empleado_inline_add.htm";
else
	$templatefile = "empleado_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing persona_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_persona_id = $pageObject->getControl("persona_id", $id);
		$control_persona_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing persona_id - end
//	processing profesion - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_profesion = $pageObject->getControl("profesion", $id);
		$control_profesion->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing profesion - end
//	processing puesto_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_puesto_id = $pageObject->getControl("puesto_id", $id);
		$control_puesto_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing puesto_id - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="persona")
	{
		...
		if($avalues["persona_id"]==""){
			$avalues["persona_id"] = prepare_for_db("persona_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="puesto")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["puesto_id"]==""){
			$avalues["puesto_id"] = prepare_for_db("puesto_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing persona_id - start
			...
			if($inlineAddOption)
			{
				$control_persona_id->afterSuccessfulSave();
			}
//	processing persona_id - end
//	processing profesion - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_profesion->afterSuccessfulSave();
			}
//	processing profesion - end
//	processing puesto_id - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_puesto_id->afterSuccessfulSave();
			}
//	processing puesto_id - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'empleado_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'empleado_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: empleado_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["persona_id"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["persona_id"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="persona")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["persona_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="puesto")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["puesto_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["profesion"]=@$avalues["profesion"];
	$defvalues["puesto_id"]=@$avalues["puesto_id"];
	$defvalues["persona_id"]=@$avalues["persona_id"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("profesion"))
		$xt->assign("profesion_fieldblock",true);
	else
		$xt->assign("profesion_tabfieldblock",true);
	$xt->assign("profesion_label",true);
	if(isEnableSection508())
		$xt->assign_section("profesion_label","<label for=\"".GetInputElementId("profesion", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("puesto_id"))
		$xt->assign("puesto_id_fieldblock",true);
	else
		$xt->assign("puesto_id_tabfieldblock",true);
	$xt->assign("puesto_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("puesto_id_label","<label for=\"".GetInputElementId("puesto_id", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("persona_id"))
		$xt->assign("persona_id_fieldblock",true);
	else
		$xt->assign("persona_id_tabfieldblock",true);
	$xt->assign("persona_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("persona_id_label","<label for=\"".GetInputElementId("persona_id", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["usuarios"]["masterkey1"] = $data["id"];	
...
//	profesion
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("profesion", $data, $keylink);
		$showValues["profesion"] = $value;
		$showFields[] = "profesion";
	}	
//	puesto_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("puesto_id", $data, $keylink);
		$showValues["puesto_id"] = $value;
		$showFields[] = "puesto_id";
	}	
//	persona_id
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("persona_id", $data, $keylink);
		$showValues["persona_id"] = $value;
		$showFields[] = "persona_id";
	}	
		$showRawValues["id"] = substr($data["id"],0,100);
		$showRawValues["profesion"] = substr($data["profesion"],0,100);
		$showRawValues["puesto_id"] = substr($data["puesto_id"],0,100);
		$showRawValues["persona_id"] = substr($data["persona_id"],0,100);
